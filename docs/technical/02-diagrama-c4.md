...
C4Container
    ...
    System_Boundary(tesseract, "Tesseract") {
        ...
        Container(actions, "Motor de Ações", "actions_catalog.py + actions_engine.js + data_binding.js", "Ações disparadas por evento de componente do Designer (Fase 10) — client-side (navigate/show_message/set_component_value/toggle_component) e server-side (call_data_action)")
    ...
    Rel(designer, actions, "DesignerComponent.events consumido pelo actions_engine.js no runtime")
    Rel(actions, odataprovider, "call_data_action chama ODataConnectionManager (Fase 10, sempre server-side)")
...
C4Component
    ...
    Container_Boundary(core, "Core") {
        ...
        Component(designer, "designer.py (controller)", "Flask", "CRUD de DesignerPage/DesignerComponent + runtime + execução de Ação de Dado")
        Component(actionscatalog, "actions_catalog.py", "Python", "Catálogo de tipos de Ação (Fase 10) — client-side e call_data_action (server-side)")
    ...
    Rel(designer, actionscatalog, "valida action_type ao gravar DesignerComponent.events")
