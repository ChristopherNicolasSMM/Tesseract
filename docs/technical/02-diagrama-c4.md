...
C4Container
    ...
    System_Boundary(tesseract, "Tesseract") {
        ...
        Container(designer, "Páginas Customizadas", "Jinja2 (content_html) + JS por página", "HTML escrito à mão (Fase 12 — o canvas drag-and-drop da Fase 10/11 foi removido, ver skill 16)")
        Container(freestyle, "Modelos Freestyle", "Jinja2 + JS por responsabilidade", "Telas de referência vivas para quem escreve página customizada (Fase 13, skill 18)")
        Container(rules, "Motor de Regras", "rule_engine.js", "Validação client-side, conectada a formulários do CrudGen")
        Container(dataaction, "Execução de Ação de Dado", "core/odata/connection_manager.py", "Único endpoint server-side que toca credencial de conexão — chamado via JS por página customizada ou freestyle (Fase 10; o catálogo de Ações client-side da Fase 10/11 foi removido junto do canvas)")
    ...
    Rel(corepages, freestyle, "/freestyle/* (Fase 13)")
    Rel(corepages, rules, "carrega rule_engine.js em formulários")
    Rel(designer, dataaction, "JS da página chama /admin/designer/data-action/<id>/execute (Fase 10)")
    Rel(freestyle, dataaction, "mesmo endpoint, via helper TesseractData (skill 18)")
    Rel(dataaction, odataprovider, "resolve ODataConnectionManager, sempre server-side")
    Rel(odataprovider, db, "query/patch em processo quando ODataConnection.is_local")
...
C4Component
    ...
    Container_Boundary(core, "Core") {
        ...
        Component(designer, "designer.py (controller)", "Flask", "CRUD de DesignerPage (content_html) + runtime + execução de Ação de Dado (Fase 12 — sem árvore de componentes)")
        Component(freestylectrl, "freestyle_model.py", "Flask", "Modelos de referência vivos — uma função por modelo, sem lógica de negócio (Fase 13, skill 18)")
        Component(odataprovidercomp, "odata_provider/", "Python", "registry.py (descoberta @odata_expose) + metadata.py (schema enriquecido) + service.py (query/patch com permissão)")
        Component(menuoverride, "designer_menu_override.py", "Python", "Resolve replace_in_menu — troca item de menu do CrudGen pela DesignerPage publicada (Fase 10)")
    ...
    Rel(designer, odataprovidercomp, "execute_data_action() chama ODataConnectionManager")
    Rel(freestylectrl, odataprovidercomp, "model_consumption/model_full chamam via mesmo endpoint")
    Rel(app_factory, menuoverride, "resolve_designer_page_menu_overrides() no boot")
