...
| **Transação** | Um ponto de entrada navegável (rota + label + ícone) exposto no menu/launcher, **ou** um nó-pasta puro (sem rota, skill 07) que agrupa outras Transações via `parent_id` — árvore de profundidade arbitrária. Todo Addon/Feature/Plugin pode declarar uma ou mais. |
