"""
addons/addon_estoque/root/api/routes/categorias_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via categorias_routes_hooks.py (nunca sobrescrito).
"""
...
categorias_api_bp = Blueprint(
    "categorias_api", __name__, url_prefix="/api/estoque/categorias"
)
...
@categorias_api_bp.route("/", methods=["GET"])
@login_required
...
@categorias_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@categorias_api_bp.route("/", methods=["POST"])
@login_required
...
@categorias_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@categorias_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@categorias_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@categorias_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
