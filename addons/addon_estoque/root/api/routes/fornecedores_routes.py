"""
addons/addon_estoque/root/api/routes/fornecedores_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via fornecedores_routes_hooks.py (nunca sobrescrito).
"""
...
fornecedores_api_bp = Blueprint(
    "fornecedores_api", __name__, url_prefix="/api/estoque/fornecedores"
)
...
@fornecedores_api_bp.route("/", methods=["GET"])
@login_required
...
@fornecedores_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@fornecedores_api_bp.route("/", methods=["POST"])
@login_required
...
@fornecedores_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@fornecedores_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@fornecedores_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@fornecedores_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
