"""
addons/addon_estoque/root/api/routes/enderecos_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via enderecos_routes_hooks.py (nunca sobrescrito).
"""
...
enderecos_api_bp = Blueprint(
    "enderecos_api", __name__, url_prefix="/api/estoque/enderecos"
)
...
@enderecos_api_bp.route("/", methods=["GET"])
@login_required
...
@enderecos_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@enderecos_api_bp.route("/", methods=["POST"])
@login_required
...
@enderecos_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@enderecos_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@enderecos_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@enderecos_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
