"""
addons/addon_estoque/root/api/routes/tipo_produtos_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via tipo_produtos_routes_hooks.py (nunca sobrescrito).
"""
...
tipo_produtos_api_bp = Blueprint(
    "tipo_produtos_api", __name__, url_prefix="/api/estoque/tipo-produtos"
)
...
@tipo_produtos_api_bp.route("/", methods=["GET"])
@login_required
...
@tipo_produtos_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@tipo_produtos_api_bp.route("/", methods=["POST"])
@login_required
...
@tipo_produtos_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@tipo_produtos_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@tipo_produtos_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@tipo_produtos_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
