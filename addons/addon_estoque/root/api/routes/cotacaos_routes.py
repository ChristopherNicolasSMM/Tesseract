"""
addons/addon_estoque/root/api/routes/cotacaos_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via cotacaos_routes_hooks.py (nunca sobrescrito).
"""
...
try:
    from addons.addon_estoque.root.controller import cotacaos_hooks as _hooks
except ImportError:
    _hooks = None
...
cotacaos_api_bp = Blueprint(
    "cotacaos_api", __name__, url_prefix="/api/estoque/cotacaos"
)
_service = CotacaoService()
...
@cotacaos_api_bp.route("/", methods=["GET"])
@login_required
...
@cotacaos_api_bp.route("/<int:id>", methods=["GET"])
@login_required
@permission_required("cotacaos.detail")
def get_item(id: int):
...
@cotacaos_api_bp.route("/", methods=["POST"])
@login_required
@permission_required("cotacaos.create")
def create_item():
...
@cotacaos_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@cotacaos_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("cotacaos.trash")
...
@cotacaos_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("cotacaos.restore")
...
@cotacaos_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
@permission_required("cotacaos.delete_permanent")
