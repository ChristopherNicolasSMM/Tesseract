"""
addons/addon_estoque/root/api/routes/item_cotacaos_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via item_cotacaos_routes_hooks.py (nunca sobrescrito).
"""
...
try:
    from addons.addon_estoque.root.controller import item_cotacaos_hooks as _hooks
except ImportError:
    _hooks = None
...
item_cotacaos_api_bp = Blueprint(
    "item_cotacaos_api", __name__, url_prefix="/api/estoque/item-cotacaos"
)
_service = ItemCotacaoService()
...
@item_cotacaos_api_bp.route("/", methods=["GET"])
@login_required
@permission_required("item_cotacaos.list")
def list_items():
    # cotacao_id / processo_cotacao_id (skill 24, Fase 6.2): a aba
    # "Itens" de uma Cotacao específica usa cotacao_id; a aba
    # "Comparação" (todas as Cotacoes do processo de uma vez) usa
    # processo_cotacao_id.
    cotacao_id = request.args.get("cotacao_id", type=int)
    processo_cotacao_id = request.args.get("processo_cotacao_id", type=int)
    items = _service.list(cotacao_id=cotacao_id, processo_cotacao_id=processo_cotacao_id)
    return _ok({"items": [i.to_dict() if hasattr(i, "to_dict") else {"id": i.id} for i in items]})


@item_cotacaos_api_bp.route("/<int:id>", methods=["GET"])
@login_required
@permission_required("item_cotacaos.detail")
def get_item(id: int):
...
@item_cotacaos_api_bp.route("/", methods=["POST"])
@login_required
@permission_required("item_cotacaos.create")
def create_item():
...
@item_cotacaos_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@item_cotacaos_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("item_cotacaos.trash")
...
@item_cotacaos_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("item_cotacaos.restore")
...
@item_cotacaos_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
@permission_required("item_cotacaos.delete_permanent")
