"""
addons/addon_estoque/root/api/routes/item_processo_cotacaos_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via item_processo_cotacaos_routes_hooks.py (nunca sobrescrito).
"""
...
try:
    from addons.addon_estoque.root.controller import item_processo_cotacaos_hooks as _hooks
except ImportError:
    _hooks = None
...
item_processo_cotacaos_api_bp = Blueprint(
    "item_processo_cotacaos_api", __name__, url_prefix="/api/estoque/item-processo-cotacaos"
)
_service = ItemProcessoCotacaoService()
...
@item_processo_cotacaos_api_bp.route("/", methods=["GET"])
@login_required
@permission_required("item_processo_cotacaos.list")
def list_items():
    # processo_cotacao_id (correção pós-Fase 6.3): filtro server-side
    # usado pela aba "Cotações" do detalhe de ProcessoCotacao.
...
@item_processo_cotacaos_api_bp.route("/<int:id>", methods=["GET"])
@login_required
@permission_required("item_processo_cotacaos.detail")
def get_item(id: int):
...
@item_processo_cotacaos_api_bp.route("/", methods=["POST"])
@login_required
@permission_required("item_processo_cotacaos.create")
def create_item():
...
@item_processo_cotacaos_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
@permission_required("item_processo_cotacaos.update")
...
@item_processo_cotacaos_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("item_processo_cotacaos.trash")
...
@item_processo_cotacaos_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("item_processo_cotacaos.restore")
...
@item_processo_cotacaos_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
@permission_required("item_processo_cotacaos.delete_permanent")
