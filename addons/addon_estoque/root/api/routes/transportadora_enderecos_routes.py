"""
addons/addon_estoque/root/api/routes/transportadora_enderecos_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via transportadora_enderecos_routes_hooks.py (nunca sobrescrito).
"""
...
transportadora_enderecos_api_bp = Blueprint(
    "transportadora_enderecos_api", __name__, url_prefix="/api/estoque/transportadora-enderecos"
)
...
@transportadora_enderecos_api_bp.route("/", methods=["GET"])
@login_required
...
@transportadora_enderecos_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@transportadora_enderecos_api_bp.route("/", methods=["POST"])
@login_required
...
@transportadora_enderecos_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@transportadora_enderecos_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@transportadora_enderecos_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@transportadora_enderecos_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
