"""
addons/addon_estoque/root/services/categoria_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(categoria_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Categoria)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in Categoria.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_estoque.root.services import categoria_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class CategoriaService:
    ...
    def list(self, *, include_deleted: bool = False):
        ...
        if not include_deleted:
            query = query.filter(Categoria.is_deleted.is_(False))
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Categoria: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Categoria id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
