"""
addons/addon_estoque/root/services/transportadora_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(transportadora_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Transportadora)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in Transportadora.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_estoque.root.services import transportadora_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class TransportadoraService:
    """Camada de negócio para Transportadora."""

    def list(self, *, include_deleted: bool = False):
        query = Transportadora.query
        if not include_deleted:
            query = query.filter(Transportadora.is_deleted.is_(False))
        return query.order_by(Transportadora.id.asc()).all()

    def get_by_id(self, id: int) -> "Transportadora | None":
        return db.session.get(Transportadora, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Transportadora: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Transportadora id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
