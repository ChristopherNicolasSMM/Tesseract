"""
addons/addon_estoque/root/services/saldo_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(saldo_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Saldo)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in Saldo.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_estoque.root.services import saldo_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class SaldoService:
    """Camada de negócio para Saldo de Estoque."""

    def list(self, *, include_deleted: bool = False):
        query = Saldo.query
        if not include_deleted:
            query = query.filter(Saldo.is_deleted.is_(False))
        return query.order_by(Saldo.id.asc()).all()

    def get_by_id(self, id: int) -> "Saldo | None":
        return db.session.get(Saldo, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Saldo: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Saldo id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
