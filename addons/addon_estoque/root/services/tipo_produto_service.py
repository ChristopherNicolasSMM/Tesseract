"""
addons/addon_estoque/root/services/tipo_produto_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(tipo_produto_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(TipoProduto)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in TipoProduto.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_estoque.root.services import tipo_produto_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class TipoProdutoService:
    """Camada de negócio para Tipo de Produto."""

    def list(self, *, include_deleted: bool = False):
        query = TipoProduto.query
        if not include_deleted:
            query = query.filter(TipoProduto.is_deleted.is_(False))
        return query.order_by(TipoProduto.id.asc()).all()

    def get_by_id(self, id: int) -> "TipoProduto | None":
        return db.session.get(TipoProduto, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar TipoProduto: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar TipoProduto id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
