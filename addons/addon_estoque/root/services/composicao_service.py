"""
addons/addon_estoque/root/services/composicao_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(composicao_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Composicao)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in Composicao.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_estoque.root.services import composicao_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class ComposicaoService:
    """Camada de negócio para Composição de Material."""

    def list(self, *, include_deleted: bool = False):
        query = Composicao.query
        if not include_deleted:
            query = query.filter(Composicao.is_deleted.is_(False))
        return query.order_by(Composicao.id.asc()).all()

    def get_by_id(self, id: int) -> "Composicao | None":
        return db.session.get(Composicao, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Composicao: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Composicao id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
