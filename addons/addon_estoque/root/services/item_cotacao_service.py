"""
addons/addon_estoque/root/services/item_cotacao_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(item_cotacao_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(ItemCotacao)

try:
    from addons.addon_estoque.root.services import item_cotacao_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class ItemCotacaoService:
    ...
    def get_by_id(self, id: int) -> "ItemCotacao | None":
        return db.session.get(ItemCotacao, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar ItemCotacao: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar ItemCotacao id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
