"""
addons/addon_estoque/root/services/material_unidade_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(material_unidade_service_hooks.py, nunca sobrescrito).
"""
...
class MaterialUnidadeService:
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar MaterialUnidade: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar MaterialUnidade id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
