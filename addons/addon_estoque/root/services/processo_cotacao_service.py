"""
addons/addon_estoque/root/services/processo_cotacao_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(processo_cotacao_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(ProcessoCotacao)

try:
    from addons.addon_estoque.root.services import processo_cotacao_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class ProcessoCotacaoService:
    """Camada de negócio para Processo de Cotação."""

    def list(self, *, include_deleted: bool = False):
        query = ProcessoCotacao.query
        if not include_deleted:
            query = query.filter(ProcessoCotacao.is_deleted.is_(False))
        return query.order_by(ProcessoCotacao.id.asc()).all()

    def get_by_id(self, id: int) -> "ProcessoCotacao | None":
        return db.session.get(ProcessoCotacao, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar ProcessoCotacao: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar ProcessoCotacao id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
