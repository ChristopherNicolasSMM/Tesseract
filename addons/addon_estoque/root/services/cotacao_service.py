"""
addons/addon_estoque/root/services/cotacao_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(cotacao_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Cotacao)

try:
    from addons.addon_estoque.root.services import cotacao_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class CotacaoService:
    ...
    def get_by_id(self, id: int) -> "Cotacao | None":
        return db.session.get(Cotacao, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Cotacao: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Cotacao id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
