"""
addons/addon_estoque/root/services/fornecedor_endereco_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(fornecedor_endereco_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(FornecedorEndereco)

try:
    from addons.addon_estoque.root.services import fornecedor_endereco_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class FornecedorEnderecoService:
    ...
    def get_by_id(self, id: int) -> "FornecedorEndereco | None":
        return db.session.get(FornecedorEndereco, id)

    def create(self, data: dict) -> ServiceResult:
        obj = FornecedorEndereco()
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar FornecedorEndereco: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar FornecedorEndereco id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
