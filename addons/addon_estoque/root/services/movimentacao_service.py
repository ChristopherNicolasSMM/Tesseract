"""
addons/addon_estoque/root/services/movimentacao_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(movimentacao_service_hooks.py, nunca sobrescrito).
"""
...
try:
    from addons.addon_estoque.root.services import movimentacao_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class MovimentacaoService:
    """Camada de negócio para Movimentação de Estoque."""

    def list(self, *, include_deleted: bool = False):
        query = Movimentacao.query
        if not include_deleted:
            query = query.filter(Movimentacao.is_deleted.is_(False))
        return query.order_by(Movimentacao.id.asc()).all()

    def get_by_id(self, id: int) -> "Movimentacao | None":
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Movimentacao: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Movimentacao id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
