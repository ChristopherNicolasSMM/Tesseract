"""
addons/addon_estoque/root/services/pedido_compra_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(pedido_compra_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(PedidoCompra)

try:
    from addons.addon_estoque.root.services import pedido_compra_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class PedidoCompraService:
    """Camada de negócio para Pedido de Compra."""

    def list(self, *, include_deleted: bool = False):
        query = PedidoCompra.query
        if not include_deleted:
            query = query.filter(PedidoCompra.is_deleted.is_(False))
        return query.order_by(PedidoCompra.id.asc()).all()

    def get_by_id(self, id: int) -> "PedidoCompra | None":
        return db.session.get(PedidoCompra, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar PedidoCompra: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar PedidoCompra id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
