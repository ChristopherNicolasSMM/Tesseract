"""
addons/addon_estoque/root/services/transportadora_endereco_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(transportadora_endereco_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(TransportadoraEndereco)

try:
    from addons.addon_estoque.root.services import transportadora_endereco_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class TransportadoraEnderecoService:
    ...
    def get_by_id(self, id: int) -> "TransportadoraEndereco | None":
        return db.session.get(TransportadoraEndereco, id)

    def create(self, data: dict) -> ServiceResult:
        obj = TransportadoraEndereco()
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar TransportadoraEndereco: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar TransportadoraEndereco id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
