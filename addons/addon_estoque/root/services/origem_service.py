"""
addons/addon_estoque/root/services/origem_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(origem_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Origem)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in Origem.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_estoque.root.services import origem_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class OrigemService:
    """Camada de negócio para Origem."""

    def list(self, *, include_deleted: bool = False):
        query = Origem.query
        if not include_deleted:
            query = query.filter(Origem.is_deleted.is_(False))
        return query.order_by(Origem.id.asc()).all()

    def get_by_id(self, id: int) -> "Origem | None":
        return db.session.get(Origem, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Origem: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Origem id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
