"""
addons/addon_estoque/root/services/fabricante_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(fabricante_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Fabricante)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in Fabricante.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_estoque.root.services import fabricante_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class FabricanteService:
    """Camada de negócio para Fabricante."""

    def list(self, *, include_deleted: bool = False):
        query = Fabricante.query
        if not include_deleted:
            query = query.filter(Fabricante.is_deleted.is_(False))
        return query.order_by(Fabricante.id.asc()).all()

    def get_by_id(self, id: int) -> "Fabricante | None":
        return db.session.get(Fabricante, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Fabricante: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Fabricante id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
