"""
addons/addon_estoque/root/services/item_processo_cotacao_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(item_processo_cotacao_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(ItemProcessoCotacao)

try:
    from addons.addon_estoque.root.services import item_processo_cotacao_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class ItemProcessoCotacaoService:
    """Camada de negócio para Item do Processo de Cotação."""

    def list(self, *, include_deleted: bool = False, processo_cotacao_id: int | None = None):
        """`processo_cotacao_id` (correção pós-Fase 6.3) — a aba
        "Cotações" precisa listar só os itens pedidos daquele processo,
        não a tabela inteira."""
        query = ItemProcessoCotacao.query
        if not include_deleted:
            query = query.filter(ItemProcessoCotacao.is_deleted.is_(False))
        if processo_cotacao_id is not None:
            query = query.filter(ItemProcessoCotacao.processo_cotacao_id == processo_cotacao_id)
        return query.order_by(ItemProcessoCotacao.id.asc()).all()

    def get_by_id(self, id: int) -> "ItemProcessoCotacao | None":
        return db.session.get(ItemProcessoCotacao, id)

    def create(self, data: dict) -> ServiceResult:
        obj = ItemProcessoCotacao()
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar ItemProcessoCotacao: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar ItemProcessoCotacao id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
