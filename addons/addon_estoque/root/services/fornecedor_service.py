"""
addons/addon_estoque/root/services/fornecedor_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(fornecedor_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Fornecedor)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in Fornecedor.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_estoque.root.services import fornecedor_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class FornecedorService:
    """Camada de negócio para Fornecedor."""

    def list(self, *, include_deleted: bool = False):
        query = Fornecedor.query
        if not include_deleted:
            query = query.filter(Fornecedor.is_deleted.is_(False))
        return query.order_by(Fornecedor.id.asc()).all()

    def get_by_id(self, id: int) -> "Fornecedor | None":
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Fornecedor: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Fornecedor id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
