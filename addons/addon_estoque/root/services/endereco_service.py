"""
addons/addon_estoque/root/services/endereco_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(endereco_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Endereco)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in Endereco.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_estoque.root.services import endereco_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class EnderecoService:
    """Camada de negócio para Endereço."""

    def list(self, *, include_deleted: bool = False):
        query = Endereco.query
        if not include_deleted:
            query = query.filter(Endereco.is_deleted.is_(False))
        return query.order_by(Endereco.id.asc()).all()

    def get_by_id(self, id: int) -> "Endereco | None":
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Endereco: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Endereco id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
