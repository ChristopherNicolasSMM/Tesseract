"""
addons/addon_estoque/root/services/material_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(material_service_hooks.py, nunca sobrescrito).
"""
...
class MaterialService:
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Material: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Material id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
