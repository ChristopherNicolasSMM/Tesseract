"""
addons/addon_estoque/root/controller/material_unidades.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via material_unidades_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
# importava/chamava material_unidades_hooks.py de verdade, só tinha o
...
material_unidades_bp = Blueprint(
    "material_unidades", __name__, url_prefix="/estoque/material-unidades"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(MaterialUnidade)
_EDITABLE_FIELDS = [c.name for c in MaterialUnidade.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in MaterialUnidade.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(MaterialUnidade) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(MaterialUnidade) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(MaterialUnidade) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(MaterialUnidade).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(MaterialUnidade, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(MaterialUnidade)

_LIST_KEY = "material_unidades"
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(MaterialUnidade, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(MaterialUnidade, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(MaterialUnidade, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(MaterialUnidade, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(MaterialUnidade, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(MaterialUnidade.query.filter(MaterialUnidade.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(MaterialUnidade.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Unidade de Material", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@material_unidades_bp.route("/", methods=["GET"])
@login_required
@permission_required("material_unidades.list")
def manage():
    return render_template("material_unidades/manage.html", **_manage_context())


@material_unidades_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("material_unidades.list")
def save_column_prefs():
    ...
    return redirect(url_for("material_unidades.manage"))


@material_unidades_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("material_unidades.list")
def export_csv():
    query = _apply_filters(MaterialUnidade.query.filter(MaterialUnidade.is_deleted.is_(False)))
    items = query.order_by(MaterialUnidade.id.desc()).all()
...
@material_unidades_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("material_unidades.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Unidade de Material"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Unidade de Material", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@material_unidades_bp.route("/<int:id>", methods=["GET"])
@login_required
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("material_unidades.manage"))
    return render_template("material_unidades/detail.html", **_detail_context(item))
...
@material_unidades_bp.route("/", methods=["POST"])
@login_required
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar MaterialUnidade: %s", e)
...
@material_unidades_bp.route("/<int:id>", methods=["POST"])
@login_required
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar MaterialUnidade id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "material_unidades/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("material_unidades.detail", id=id))


@material_unidades_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@material_unidades_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@material_unidades_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
