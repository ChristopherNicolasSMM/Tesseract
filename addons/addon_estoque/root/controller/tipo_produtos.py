"""
addons/addon_estoque/root/controller/tipo_produtos.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via tipo_produtos_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
from addons.addon_estoque.root.services.tipo_produto_service import TipoProdutoService
...
# importava/chamava tipo_produtos_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_estoque.root.controller import tipo_produtos_hooks as _hooks
except ImportError:
    _hooks = None
...
tipo_produtos_bp = Blueprint(
    "tipo_produtos", __name__, url_prefix="/estoque/tipo-produtos"
)
_service = TipoProdutoService()
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(TipoProduto)
_EDITABLE_FIELDS = [c.name for c in TipoProduto.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in TipoProduto.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(TipoProduto) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(TipoProduto) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(TipoProduto) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(TipoProduto).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(TipoProduto, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(TipoProduto)

_LIST_KEY = "tipo_produtos"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(TipoProduto, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(TipoProduto, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(TipoProduto, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(TipoProduto, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(TipoProduto, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(TipoProduto, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(TipoProduto.query.filter(TipoProduto.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(TipoProduto.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Tipo de Produto", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@tipo_produtos_bp.route("/", methods=["GET"])
@login_required
@permission_required("tipo_produtos.list")
def manage():
    return render_template("tipo_produtos/manage.html", **_manage_context())


@tipo_produtos_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("tipo_produtos.list")
def save_column_prefs():
    ...
    return redirect(url_for("tipo_produtos.manage"))


@tipo_produtos_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("tipo_produtos.list")
def export_csv():
    query = _apply_filters(TipoProduto.query.filter(TipoProduto.is_deleted.is_(False)))
    items = query.order_by(TipoProduto.id.desc()).all()
...
@tipo_produtos_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("tipo_produtos.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Tipo de Produto"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Tipo de Produto", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@tipo_produtos_bp.route("/<int:id>", methods=["GET"])
@login_required
@permission_required("tipo_produtos.detail")
def detail(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
        return redirect(url_for("tipo_produtos.manage"))
    return render_template("tipo_produtos/detail.html", **_detail_context(item))
...
@tipo_produtos_bp.route("/", methods=["POST"])
@login_required
@permission_required("tipo_produtos.create")
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar TipoProduto: %s", e)
...
@tipo_produtos_bp.route("/<int:id>", methods=["POST"])
@login_required
@permission_required("tipo_produtos.update")
def update(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar TipoProduto id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "tipo_produtos/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("tipo_produtos.detail", id=id))


@tipo_produtos_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("tipo_produtos.trash")
def trash(id: int):
...
@tipo_produtos_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("tipo_produtos.restore")
def restore(id: int):
...
@tipo_produtos_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
@permission_required("tipo_produtos.delete_permanent")
...
@tipo_produtos_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("tipo_produtos.trash")
def bulk_trash():
...
@tipo_produtos_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("tipo_produtos.update")
def bulk_inactivate():
    from flask import jsonify
