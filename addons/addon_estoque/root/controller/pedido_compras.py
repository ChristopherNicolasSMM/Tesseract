"""
addons/addon_estoque/root/controller/pedido_compras.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via pedido_compras_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
# importava/chamava pedido_compras_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_estoque.root.controller import pedido_compras_hooks as _hooks
except ImportError:
    _hooks = None
...
pedido_compras_bp = Blueprint(
    "pedido_compras", __name__, url_prefix="/estoque/pedido-compras"
)
_service = PedidoCompraService()
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(PedidoCompra)
_EDITABLE_FIELDS = [c.name for c in PedidoCompra.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in PedidoCompra.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(PedidoCompra) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(PedidoCompra) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(PedidoCompra) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(PedidoCompra).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(PedidoCompra, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(PedidoCompra)

_LIST_KEY = "pedido_compras"
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(PedidoCompra, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(PedidoCompra, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(PedidoCompra, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(PedidoCompra, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(PedidoCompra, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(PedidoCompra.query.filter(PedidoCompra.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(PedidoCompra.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Pedido de Compra", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@pedido_compras_bp.route("/", methods=["GET"])
@login_required
@permission_required("pedido_compras.list")
def manage():
    return render_template("pedido_compras/manage.html", **_manage_context())


@pedido_compras_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("pedido_compras.list")
def save_column_prefs():
    ...
    return redirect(url_for("pedido_compras.manage"))


@pedido_compras_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("pedido_compras.list")
def export_csv():
    query = _apply_filters(PedidoCompra.query.filter(PedidoCompra.is_deleted.is_(False)))
    items = query.order_by(PedidoCompra.id.desc()).all()
...
@pedido_compras_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("pedido_compras.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Pedido de Compra"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Pedido de Compra", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@pedido_compras_bp.route("/<int:id>", methods=["GET"])
@login_required
@permission_required("pedido_compras.detail")
def detail(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
        return redirect(url_for("pedido_compras.manage"))
    return render_template("pedido_compras/detail.html", **_detail_context(item))
...
@pedido_compras_bp.route("/", methods=["POST"])
@login_required
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar PedidoCompra: %s", e)
...
@pedido_compras_bp.route("/<int:id>", methods=["POST"])
@login_required
...
@pedido_compras_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("pedido_compras.trash")
def trash(id: int):
...
@pedido_compras_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("pedido_compras.restore")
def restore(id: int):
...
@pedido_compras_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
@permission_required("pedido_compras.delete_permanent")
