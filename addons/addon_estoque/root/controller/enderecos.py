"""
addons/addon_estoque/root/controller/enderecos.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via enderecos_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
from addons.addon_estoque.root.services.endereco_service import EnderecoService
...
# importava/chamava enderecos_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_estoque.root.controller import enderecos_hooks as _hooks
except ImportError:
    _hooks = None
...
enderecos_bp = Blueprint(
    "enderecos", __name__, url_prefix="/estoque/enderecos"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Endereco)
_EDITABLE_FIELDS = [c.name for c in Endereco.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in Endereco.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(Endereco) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(Endereco) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(Endereco) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(Endereco).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(Endereco, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(Endereco)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(Endereco, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(Endereco, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(Endereco, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(Endereco, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(Endereco, _ef["field"]) == value)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(Endereco.query.filter(Endereco.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(Endereco.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Endereço", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
def manage():
    return render_template("enderecos/manage.html", **_manage_context())


@enderecos_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("enderecos.list")
def save_column_prefs():
    ...
    return redirect(url_for("enderecos.manage"))


@enderecos_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("enderecos.list")
def export_csv():
    query = _apply_filters(Endereco.query.filter(Endereco.is_deleted.is_(False)))
    items = query.order_by(Endereco.id.desc()).all()
...
@enderecos_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("enderecos.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Endereço"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Endereço", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@enderecos_bp.route("/<int:id>", methods=["GET"])
@login_required
@permission_required("enderecos.detail")
def detail(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
        return redirect(url_for("enderecos.manage"))
    return render_template("enderecos/detail.html", **_detail_context(item))
...
@login_required
@permission_required("enderecos.create")
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar Endereco: %s", e)
...
@enderecos_bp.route("/<int:id>", methods=["POST"])
@login_required
@permission_required("enderecos.update")
def update(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar Endereco id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "enderecos/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("enderecos.detail", id=id))


@enderecos_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("enderecos.trash")
def trash(id: int):
...
@enderecos_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("enderecos.restore")
def restore(id: int):
...
@enderecos_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
@permission_required("enderecos.delete_permanent")
...
@enderecos_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("enderecos.trash")
def bulk_trash():
...
@enderecos_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("enderecos.update")
def bulk_inactivate():
    from flask import jsonify
