"""
addons/addon_estoque/root/controller/categorias.py
...
# importava/chamava categorias_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_estoque.root.controller import categorias_hooks as _hooks
except ImportError:
    _hooks = None
...
categorias_bp = Blueprint(
    "categorias", __name__, url_prefix="/estoque/categorias"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Categoria)
_EDITABLE_FIELDS = [c.name for c in Categoria.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in Categoria.__table__.columns
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(Categoria, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(Categoria, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(Categoria, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(Categoria, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(Categoria, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(Categoria, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(Categoria.query.filter(Categoria.is_deleted.is_(False)))
...
@categorias_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("categorias.list")
def export_csv():
    query = _apply_filters(Categoria.query.filter(Categoria.is_deleted.is_(False)))
    items = query.order_by(Categoria.id.desc()).all()
...
@categorias_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("categorias.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Categoria"[:31]  # limite do Excel pro nome da aba
...
@login_required
@permission_required("categorias.detail")
def detail(id: int):
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar Categoria: %s", e)
...
@categorias_bp.route("/<int:id>", methods=["POST"])
@login_required
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar Categoria id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@categorias_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("categorias.trash")
def trash(id: int):
...
@categorias_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("categorias.restore")
def restore(id: int):
...
@categorias_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
@permission_required("categorias.delete_permanent")
...
@categorias_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("categorias.trash")
def bulk_trash():
...
@categorias_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("categorias.update")
def bulk_inactivate():
    from flask import jsonify
