"""
addons/addon_estoque/root/controller/origems.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via origems_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
# importava/chamava origems_hooks.py de verdade, só tinha o
...
origems_bp = Blueprint(
    "origems", __name__, url_prefix="/estoque/origems"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Origem)
_EDITABLE_FIELDS = [c.name for c in Origem.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in Origem.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(Origem) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(Origem) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(Origem) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(Origem).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(Origem, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(Origem)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(Origem, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(Origem, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(Origem, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(Origem, _ef["field"]) == value)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(Origem.query.filter(Origem.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(Origem.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Origem", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@origems_bp.route("/", methods=["GET"])
@login_required
@permission_required("origems.list")
def manage():
    return render_template("origems/manage.html", **_manage_context())


@origems_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("origems.list")
def save_column_prefs():
    ...
    return redirect(url_for("origems.manage"))


@origems_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("origems.list")
def export_csv():
    query = _apply_filters(Origem.query.filter(Origem.is_deleted.is_(False)))
    items = query.order_by(Origem.id.desc()).all()
...
@origems_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("origems.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Origem"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Origem", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@origems_bp.route("/<int:id>", methods=["GET"])
@login_required
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("origems.manage"))
    return render_template("origems/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar Origem: %s", e)
...
@origems_bp.route("/<int:id>", methods=["POST"])
@login_required
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar Origem id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
    ...
    return redirect(url_for("origems.detail", id=id))


@origems_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@origems_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@origems_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
...
@origems_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("origems.trash")
def bulk_trash():
...
@origems_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("origems.update")
def bulk_inactivate():
    from flask import jsonify
