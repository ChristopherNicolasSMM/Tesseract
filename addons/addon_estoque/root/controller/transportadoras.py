"""
addons/addon_estoque/root/controller/transportadoras.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via transportadoras_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
from addons.addon_estoque.root.services.transportadora_service import TransportadoraService
...
# importava/chamava transportadoras_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_estoque.root.controller import transportadoras_hooks as _hooks
except ImportError:
    _hooks = None
...
transportadoras_bp = Blueprint(
    "transportadoras", __name__, url_prefix="/estoque/transportadoras"
)
_service = TransportadoraService()
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Transportadora)
_EDITABLE_FIELDS = [c.name for c in Transportadora.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in Transportadora.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(Transportadora) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(Transportadora) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(Transportadora) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(Transportadora).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(Transportadora, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(Transportadora)

_LIST_KEY = "transportadoras"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(Transportadora, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(Transportadora, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(Transportadora, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(Transportadora, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(Transportadora, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(Transportadora, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(Transportadora.query.filter(Transportadora.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(Transportadora.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Transportadora", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
def manage():
    return render_template("transportadoras/manage.html", **_manage_context())


@transportadoras_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("transportadoras.list")
def save_column_prefs():
    ...
    return redirect(url_for("transportadoras.manage"))


@transportadoras_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("transportadoras.list")
def export_csv():
    query = _apply_filters(Transportadora.query.filter(Transportadora.is_deleted.is_(False)))
    items = query.order_by(Transportadora.id.desc()).all()
...
@transportadoras_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("transportadoras.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Transportadora"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Transportadora", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@transportadoras_bp.route("/<int:id>", methods=["GET"])
@login_required
@permission_required("transportadoras.detail")
def detail(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
        return redirect(url_for("transportadoras.manage"))
    return render_template("transportadoras/detail.html", **_detail_context(item))
...
@login_required
@permission_required("transportadoras.create")
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar Transportadora: %s", e)
...
@transportadoras_bp.route("/<int:id>", methods=["POST"])
@login_required
@permission_required("transportadoras.update")
def update(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar Transportadora id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "transportadoras/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("transportadoras.detail", id=id))


@transportadoras_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("transportadoras.trash")
def trash(id: int):
...
@transportadoras_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("transportadoras.restore")
def restore(id: int):
...
@transportadoras_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
@permission_required("transportadoras.delete_permanent")
...
@transportadoras_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("transportadoras.trash")
def bulk_trash():
...
@transportadoras_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("transportadoras.update")
def bulk_inactivate():
    from flask import jsonify
