...
# importava/chamava saldos_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_estoque.root.controller import saldos_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Saldo)
...
def _apply_filters(query):
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(Saldo, _ef["field"]) == value)
...
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Saldo de Estoque"[:31]  # limite do Excel pro nome da aba
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar Saldo: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar Saldo id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@saldos_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("saldos.trash")
def bulk_trash():
...
@saldos_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("saldos.update")
def bulk_inactivate():
    from flask import jsonify
