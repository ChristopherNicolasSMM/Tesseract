"""
addons/addon_estoque/root/controller/processo_cotacaos.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via processo_cotacaos_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
# importava/chamava processo_cotacaos_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_estoque.root.controller import processo_cotacaos_hooks as _hooks
except ImportError:
    _hooks = None
...
processo_cotacaos_bp = Blueprint(
    "processo_cotacaos", __name__, url_prefix="/estoque/processo-cotacaos"
)
_service = ProcessoCotacaoService()
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(ProcessoCotacao)
_EDITABLE_FIELDS = [c.name for c in ProcessoCotacao.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in ProcessoCotacao.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(ProcessoCotacao) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(ProcessoCotacao) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(ProcessoCotacao) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(ProcessoCotacao).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(ProcessoCotacao, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(ProcessoCotacao)

_LIST_KEY = "processo_cotacaos"
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(ProcessoCotacao, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(ProcessoCotacao, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(ProcessoCotacao, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(ProcessoCotacao, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(ProcessoCotacao, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(ProcessoCotacao.query.filter(ProcessoCotacao.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(ProcessoCotacao.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Processo de Cotação", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@processo_cotacaos_bp.route("/", methods=["GET"])
@login_required
@permission_required("processo_cotacaos.list")
def manage():
    return render_template("processo_cotacaos/manage.html", **_manage_context())


@processo_cotacaos_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("processo_cotacaos.list")
def save_column_prefs():
    ...
    return redirect(url_for("processo_cotacaos.manage"))


@processo_cotacaos_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("processo_cotacaos.list")
def export_csv():
    query = _apply_filters(ProcessoCotacao.query.filter(ProcessoCotacao.is_deleted.is_(False)))
    items = query.order_by(ProcessoCotacao.id.desc()).all()
...
@processo_cotacaos_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("processo_cotacaos.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Processo de Cotação"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Processo de Cotação", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@processo_cotacaos_bp.route("/<int:id>", methods=["GET"])
@login_required
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("processo_cotacaos.manage"))
    return render_template("processo_cotacaos/detail.html", **_detail_context(item))
...
@processo_cotacaos_bp.route("/", methods=["POST"])
@login_required
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar ProcessoCotacao: %s", e)
...
@processo_cotacaos_bp.route("/<int:id>", methods=["POST"])
@login_required
...
@processo_cotacaos_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("processo_cotacaos.trash")
def trash(id: int):
...
@processo_cotacaos_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("processo_cotacaos.restore")
def restore(id: int):
...
@processo_cotacaos_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
@permission_required("processo_cotacaos.delete_permanent")
