...
# importava/chamava materials_hooks.py de verdade, só tinha o
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Material)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(Material, id)
...
def _apply_filters(query):
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(Material, _ef["field"]) == value)
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar Material: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar Material id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@materials_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("materials.trash")
def bulk_trash():
...
@materials_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("materials.update")
def bulk_inactivate():
    from flask import jsonify
