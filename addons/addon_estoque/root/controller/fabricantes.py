"""
addons/addon_estoque/root/controller/fabricantes.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via fabricantes_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
# importava/chamava fabricantes_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_estoque.root.controller import fabricantes_hooks as _hooks
except ImportError:
    _hooks = None
...
fabricantes_bp = Blueprint(
    "fabricantes", __name__, url_prefix="/estoque/fabricantes"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Fabricante)
_EDITABLE_FIELDS = [c.name for c in Fabricante.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in Fabricante.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(Fabricante) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(Fabricante) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(Fabricante) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(Fabricante).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(Fabricante, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(Fabricante)

_LIST_KEY = "fabricantes"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(Fabricante, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(Fabricante, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(Fabricante, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(Fabricante, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(Fabricante, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(Fabricante, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(Fabricante.query.filter(Fabricante.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(Fabricante.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Fabricante", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@fabricantes_bp.route("/", methods=["GET"])
@login_required
@permission_required("fabricantes.list")
def manage():
    return render_template("fabricantes/manage.html", **_manage_context())


@fabricantes_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("fabricantes.list")
def save_column_prefs():
    ...
    return redirect(url_for("fabricantes.manage"))


@fabricantes_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("fabricantes.list")
def export_csv():
    query = _apply_filters(Fabricante.query.filter(Fabricante.is_deleted.is_(False)))
    items = query.order_by(Fabricante.id.desc()).all()
...
@fabricantes_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("fabricantes.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Fabricante"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Fabricante", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@fabricantes_bp.route("/<int:id>", methods=["GET"])
@login_required
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("fabricantes.manage"))
    return render_template("fabricantes/detail.html", **_detail_context(item))
...
@fabricantes_bp.route("/", methods=["POST"])
@login_required
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar Fabricante: %s", e)
...
@fabricantes_bp.route("/<int:id>", methods=["POST"])
@login_required
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar Fabricante id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "fabricantes/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("fabricantes.detail", id=id))


@fabricantes_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@fabricantes_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@fabricantes_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
...
@fabricantes_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("fabricantes.trash")
def bulk_trash():
...
@fabricantes_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("fabricantes.update")
def bulk_inactivate():
    from flask import jsonify
