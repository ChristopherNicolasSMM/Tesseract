"""
addons/addon_estoque/root/controller/transportadora_enderecos.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via transportadora_enderecos_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
from addons.addon_estoque.root.services.transportadora_endereco_service import TransportadoraEnderecoService
...
# importava/chamava transportadora_enderecos_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_estoque.root.controller import transportadora_enderecos_hooks as _hooks
except ImportError:
    _hooks = None
...
transportadora_enderecos_bp = Blueprint(
    "transportadora_enderecos", __name__, url_prefix="/estoque/transportadora-enderecos"
)
_service = TransportadoraEnderecoService()
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(TransportadoraEndereco)
_EDITABLE_FIELDS = [c.name for c in TransportadoraEndereco.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in TransportadoraEndereco.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(TransportadoraEndereco) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(TransportadoraEndereco) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(TransportadoraEndereco) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(TransportadoraEndereco).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(TransportadoraEndereco, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(TransportadoraEndereco)

_LIST_KEY = "transportadora_enderecos"
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(TransportadoraEndereco, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(TransportadoraEndereco, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(TransportadoraEndereco, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(TransportadoraEndereco, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(TransportadoraEndereco, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(TransportadoraEndereco.query.filter(TransportadoraEndereco.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(TransportadoraEndereco.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Endereço da Transportadora", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@transportadora_enderecos_bp.route("/", methods=["GET"])
@login_required
@permission_required("transportadora_enderecos.list")
def manage():
    return render_template("transportadora_enderecos/manage.html", **_manage_context())


@transportadora_enderecos_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("transportadora_enderecos.list")
def save_column_prefs():
    ...
    return redirect(url_for("transportadora_enderecos.manage"))


@transportadora_enderecos_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("transportadora_enderecos.list")
def export_csv():
    query = _apply_filters(TransportadoraEndereco.query.filter(TransportadoraEndereco.is_deleted.is_(False)))
    items = query.order_by(TransportadoraEndereco.id.desc()).all()
...
@transportadora_enderecos_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("transportadora_enderecos.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Endereço da Transportadora"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Endereço da Transportadora", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@transportadora_enderecos_bp.route("/<int:id>", methods=["GET"])
@login_required
@permission_required("transportadora_enderecos.detail")
def detail(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
        return redirect(url_for("transportadora_enderecos.manage"))
    return render_template("transportadora_enderecos/detail.html", **_detail_context(item))
...
@transportadora_enderecos_bp.route("/", methods=["POST"])
@login_required
@permission_required("transportadora_enderecos.create")
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar TransportadoraEndereco: %s", e)
    ...
    if not success:
        ...
        return render_template(
            "transportadora_enderecos/manage.html",
...
@transportadora_enderecos_bp.route("/<int:id>", methods=["POST"])
@login_required
@permission_required("transportadora_enderecos.update")
def update(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar TransportadoraEndereco id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "transportadora_enderecos/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("transportadora_enderecos.detail", id=id))


@transportadora_enderecos_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("transportadora_enderecos.trash")
def trash(id: int):
...
@transportadora_enderecos_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("transportadora_enderecos.restore")
def restore(id: int):
...
@transportadora_enderecos_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
@permission_required("transportadora_enderecos.delete_permanent")
