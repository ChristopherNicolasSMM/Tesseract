"""
addons/addon_estoque/root/controller/composicaos.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via composicaos_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
# importava/chamava composicaos_hooks.py de verdade, só tinha o
...
composicaos_bp = Blueprint(
    "composicaos", __name__, url_prefix="/estoque/composicaos"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Composicao)
_EDITABLE_FIELDS = [c.name for c in Composicao.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in Composicao.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(Composicao) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(Composicao) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(Composicao) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(Composicao).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(Composicao, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(Composicao)

_LIST_KEY = "composicaos"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(Composicao, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(Composicao, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(Composicao, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(Composicao, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(Composicao, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(Composicao, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(Composicao.query.filter(Composicao.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(Composicao.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Composição de Material", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@composicaos_bp.route("/", methods=["GET"])
@login_required
@permission_required("composicaos.list")
def manage():
    return render_template("composicaos/manage.html", **_manage_context())


@composicaos_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("composicaos.list")
def save_column_prefs():
    ...
    return redirect(url_for("composicaos.manage"))


@composicaos_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("composicaos.list")
def export_csv():
    query = _apply_filters(Composicao.query.filter(Composicao.is_deleted.is_(False)))
    items = query.order_by(Composicao.id.desc()).all()
...
@composicaos_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("composicaos.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Composição de Material"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Composição de Material", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@composicaos_bp.route("/<int:id>", methods=["GET"])
@login_required
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("composicaos.manage"))
    return render_template("composicaos/detail.html", **_detail_context(item))
...
@composicaos_bp.route("/", methods=["POST"])
@login_required
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar Composicao: %s", e)
...
@composicaos_bp.route("/<int:id>", methods=["POST"])
@login_required
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar Composicao id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "composicaos/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("composicaos.detail", id=id))


@composicaos_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@composicaos_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@composicaos_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
...
@composicaos_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("composicaos.trash")
def bulk_trash():
...
@composicaos_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("composicaos.update")
def bulk_inactivate():
    from flask import jsonify
