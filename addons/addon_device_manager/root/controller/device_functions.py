"""
addons/addon_device_manager/root/controller/device_functions.py
...
# importava/chamava device_functions_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_device_manager.root.controller import device_functions_hooks as _hooks
except ImportError:
    _hooks = None
...
device_functions_bp = Blueprint(
    "device_functions", __name__, url_prefix="/device-manager/device-functions"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(DeviceFunction)
...
_WEAK_REFS = [wr for wr in get_weak_refs(DeviceFunction) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(DeviceFunction) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(DeviceFunction).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(DeviceFunction, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(DeviceFunction)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(DeviceFunction, id)
...
def _apply_filters(query):
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(DeviceFunction, _ef["field"]) == value)
...
def manage():
    return render_template("device_functions/manage.html", **_manage_context())
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("device_functions.manage"))
    return render_template("device_functions/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar DeviceFunction: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar DeviceFunction id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@device_functions_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("device_functions.trash")
def bulk_trash():
...
@device_functions_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("device_functions.update")
def bulk_inactivate():
    from flask import jsonify
