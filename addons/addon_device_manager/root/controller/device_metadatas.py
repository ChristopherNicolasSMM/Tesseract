"""
addons/addon_device_manager/root/controller/device_metadatas.py
...
# importava/chamava device_metadatas_hooks.py de verdade, só tinha o
...
device_metadatas_bp = Blueprint(
    "device_metadatas", __name__, url_prefix="/device-manager/device-metadatas"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(DeviceMetadata)
...
_WEAK_REFS = [wr for wr in get_weak_refs(DeviceMetadata) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(DeviceMetadata) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(DeviceMetadata).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(DeviceMetadata, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(DeviceMetadata)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(DeviceMetadata, id)
...
def _apply_filters(query):
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(DeviceMetadata, _ef["field"]) == value)
...
def manage():
    return render_template("device_metadatas/manage.html", **_manage_context())
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("device_metadatas.manage"))
    return render_template("device_metadatas/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar DeviceMetadata: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar DeviceMetadata id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@device_metadatas_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("device_metadatas.trash")
def bulk_trash():
...
@device_metadatas_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("device_metadatas.update")
def bulk_inactivate():
    from flask import jsonify
