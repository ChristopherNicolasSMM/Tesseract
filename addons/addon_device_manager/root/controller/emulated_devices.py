"""
addons/addon_device_manager/root/controller/emulated_devices.py
...
# importava/chamava emulated_devices_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_device_manager.root.controller import emulated_devices_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(EmulatedDevice)
...
_WEAK_REFS = [wr for wr in get_weak_refs(EmulatedDevice) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(EmulatedDevice) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(EmulatedDevice).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(EmulatedDevice, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(EmulatedDevice)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(EmulatedDevice, id)
...
def _apply_filters(query):
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(EmulatedDevice, _ef["field"]) == value)
...
def manage():
    return render_template("emulated_devices/manage.html", **_manage_context())
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("emulated_devices.manage"))
    return render_template("emulated_devices/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar EmulatedDevice: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar EmulatedDevice id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@emulated_devices_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("emulated_devices.trash")
def bulk_trash():
...
@emulated_devices_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("emulated_devices.update")
def bulk_inactivate():
    from flask import jsonify
