"""
addons/addon_device_manager/root/services/device_function_service.py
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(DeviceFunction)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in DeviceFunction.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_device_manager.root.services import device_function_service_hooks as _hooks
except ImportError:
    _hooks = None
