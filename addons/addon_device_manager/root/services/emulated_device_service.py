"""
addons/addon_device_manager/root/services/emulated_device_service.py
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(EmulatedDevice)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in EmulatedDevice.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_device_manager.root.services import emulated_device_service_hooks as _hooks
except ImportError:
    _hooks = None
