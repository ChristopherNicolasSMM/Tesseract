"""
addons/addon_brewstation/features/feature_envase/services/envase_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(envase_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Envase)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in Envase.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_envase.services import envase_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class EnvaseService:
    """Camada de negócio para Envase."""

    def list(self, *, include_deleted: bool = False):
        query = Envase.query
        if not include_deleted:
            query = query.filter(Envase.is_deleted.is_(False))
        return query.order_by(Envase.id.asc()).all()

    def get_by_id(self, id: int) -> "Envase | None":
        return db.session.get(Envase, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Envase: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Envase id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
