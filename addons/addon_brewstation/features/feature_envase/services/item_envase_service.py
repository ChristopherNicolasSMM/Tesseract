"""
addons/addon_brewstation/features/feature_envase/services/item_envase_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(item_envase_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(ItemEnvase)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in ItemEnvase.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_envase.services import item_envase_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class ItemEnvaseService:
    """Camada de negócio para Item de Envase."""

    def list(self, *, include_deleted: bool = False):
        query = ItemEnvase.query
        if not include_deleted:
            query = query.filter(ItemEnvase.is_deleted.is_(False))
        return query.order_by(ItemEnvase.id.asc()).all()

    def get_by_id(self, id: int) -> "ItemEnvase | None":
        return db.session.get(ItemEnvase, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar ItemEnvase: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar ItemEnvase id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
