"""
addons/addon_brewstation/features/feature_envase/controller/envases.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via envases_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
try:
    from addons.addon_brewstation.features.feature_envase.controller import envases_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Envase)
_EDITABLE_FIELDS = [c.name for c in Envase.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in Envase.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(Envase) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(Envase) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(Envase) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(Envase).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(Envase, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(Envase)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(Envase, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(Envase, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(Envase, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(Envase, _ef["field"]) == value)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(Envase.query.filter(Envase.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(Envase.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Envase", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@envases_bp.route("/", methods=["GET"])
@login_required
@permission_required("envases.list")
def manage():
    return render_template("envases/manage.html", **_manage_context())


@envases_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("envases.list")
def save_column_prefs():
    ...
    return redirect(url_for("envases.manage"))


@envases_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("envases.list")
def export_csv():
    query = _apply_filters(Envase.query.filter(Envase.is_deleted.is_(False)))
    items = query.order_by(Envase.id.desc()).all()
...
@envases_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("envases.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Envase"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Envase", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@envases_bp.route("/<int:id>", methods=["GET"])
@login_required
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("envases.manage"))
    return render_template("envases/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar Envase: %s", e)
...
@envases_bp.route("/<int:id>", methods=["POST"])
@login_required
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar Envase id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@envases_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("envases.trash")
def trash(id: int):
...
@envases_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("envases.restore")
def restore(id: int):
...
@envases_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
@permission_required("envases.delete_permanent")
...
@envases_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("envases.trash")
def bulk_trash():
...
@envases_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("envases.update")
def bulk_inactivate():
    from flask import jsonify
