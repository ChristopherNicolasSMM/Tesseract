...
# importava/chamava item_envases_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_envase.controller import item_envases_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(ItemEnvase)
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(ItemEnvase) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(ItemEnvase).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(ItemEnvase, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(ItemEnvase)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(ItemEnvase, id)
...
def manage():
    return render_template("item_envases/manage.html", **_manage_context())
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("item_envases.manage"))
    return render_template("item_envases/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar ItemEnvase: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar ItemEnvase id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@item_envases_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("item_envases.trash")
def bulk_trash():
...
@item_envases_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("item_envases.update")
def bulk_inactivate():
    from flask import jsonify
