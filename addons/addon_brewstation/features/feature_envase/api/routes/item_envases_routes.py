"""
addons/addon_brewstation/features/feature_envase/api/routes/item_envases_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via item_envases_routes_hooks.py (nunca sobrescrito).
"""
...
item_envases_api_bp = Blueprint(
    "item_envases_api", __name__, url_prefix="/api/brewstation/item-envases"
)
...
@item_envases_api_bp.route("/", methods=["GET"])
@login_required
...
@item_envases_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@item_envases_api_bp.route("/", methods=["POST"])
@login_required
...
@item_envases_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@item_envases_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@item_envases_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@item_envases_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
