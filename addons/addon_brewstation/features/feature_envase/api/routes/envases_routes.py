"""
addons/addon_brewstation/features/feature_envase/api/routes/envases_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via envases_routes_hooks.py (nunca sobrescrito).
"""
...
envases_api_bp = Blueprint(
    "envases_api", __name__, url_prefix="/api/brewstation/envases"
)
...
@envases_api_bp.route("/", methods=["GET"])
@login_required
...
@envases_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@envases_api_bp.route("/", methods=["POST"])
@login_required
...
@envases_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@envases_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@envases_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@envases_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
