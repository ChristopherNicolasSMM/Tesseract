"""
addons/addon_brewstation/features/feature_yeast_bank/api/routes/yeast_starter_logs_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via yeast_starter_logs_routes_hooks.py (nunca sobrescrito).
"""
...
yeast_starter_logs_api_bp = Blueprint(
    "yeast_starter_logs_api", __name__, url_prefix="/api/brewstation/yeast-starter-logs"
)
...
@yeast_starter_logs_api_bp.route("/", methods=["GET"])
@login_required
...
@yeast_starter_logs_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@yeast_starter_logs_api_bp.route("/", methods=["POST"])
@login_required
...
@yeast_starter_logs_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@yeast_starter_logs_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@yeast_starter_logs_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@yeast_starter_logs_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
