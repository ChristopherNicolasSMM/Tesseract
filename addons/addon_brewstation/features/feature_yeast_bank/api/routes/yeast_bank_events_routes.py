"""
addons/addon_brewstation/features/feature_yeast_bank/api/routes/yeast_bank_events_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via yeast_bank_events_routes_hooks.py (nunca sobrescrito).
"""
...
yeast_bank_events_api_bp = Blueprint(
    "yeast_bank_events_api", __name__, url_prefix="/api/brewstation/yeast-bank-events"
)
...
@yeast_bank_events_api_bp.route("/", methods=["GET"])
@login_required
...
@yeast_bank_events_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@yeast_bank_events_api_bp.route("/", methods=["POST"])
@login_required
...
@yeast_bank_events_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@yeast_bank_events_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@yeast_bank_events_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@yeast_bank_events_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
