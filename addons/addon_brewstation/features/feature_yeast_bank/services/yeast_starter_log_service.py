...
try:
    from addons.addon_brewstation.features.feature_yeast_bank.services import yeast_starter_log_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class YeastStarterLogService:
    """Camada de negócio para Starter."""

    def list(self, *, include_deleted: bool = False):
        query = YeastStarterLog.query
        if not include_deleted:
            query = query.filter(YeastStarterLog.is_deleted.is_(False))
        return query.order_by(YeastStarterLog.id.asc()).all()

    def get_by_id(self, id: int) -> "YeastStarterLog | None":
        return db.session.get(YeastStarterLog, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar YeastStarterLog: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar YeastStarterLog id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
