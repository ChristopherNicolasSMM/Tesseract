...
try:
    from addons.addon_brewstation.features.feature_yeast_bank.services import yeast_storage_reading_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class YeastStorageReadingService:
    """Camada de negócio para Leitura de Armazenamento."""

    def list(self, *, include_deleted: bool = False):
        query = YeastStorageReading.query
        if not include_deleted:
            query = query.filter(YeastStorageReading.is_deleted.is_(False))
        return query.order_by(YeastStorageReading.id.asc()).all()

    def get_by_id(self, id: int) -> "YeastStorageReading | None":
        return db.session.get(YeastStorageReading, id)

    def create(self, data: dict) -> ServiceResult:
        obj = YeastStorageReading()
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar YeastStorageReading: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar YeastStorageReading id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
