"""
addons/addon_brewstation/features/feature_yeast_bank/services/yeast_cell_count_history_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(yeast_cell_count_history_service_hooks.py, nunca sobrescrito).
"""
...
class YeastCellCountHistoryService:
    """Camada de negócio para Histórico de Contagem."""

    def list(self, *, include_deleted: bool = False):
        query = YeastCellCountHistory.query
        if not include_deleted:
            query = query.filter(YeastCellCountHistory.is_deleted.is_(False))
        return query.order_by(YeastCellCountHistory.id.asc()).all()

    def get_by_id(self, id: int) -> "YeastCellCountHistory | None":
        return db.session.get(YeastCellCountHistory, id)

    def create(self, data: dict) -> ServiceResult:
        obj = YeastCellCountHistory()
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar YeastCellCountHistory: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar YeastCellCountHistory id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
