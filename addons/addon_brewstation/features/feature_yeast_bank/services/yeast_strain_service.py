"""
addons/addon_brewstation/features/feature_yeast_bank/services/yeast_strain_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(yeast_strain_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(YeastStrain)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in YeastStrain.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_yeast_bank.services import yeast_strain_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class YeastStrainService:
    """Camada de negócio para Cepa de Levedura."""

    def list(self, *, include_deleted: bool = False):
        query = YeastStrain.query
        if not include_deleted:
            query = query.filter(YeastStrain.is_deleted.is_(False))
        return query.order_by(YeastStrain.id.asc()).all()

    def get_by_id(self, id: int) -> "YeastStrain | None":
        return db.session.get(YeastStrain, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar YeastStrain: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar YeastStrain id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
