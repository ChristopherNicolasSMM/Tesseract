...
# importava/chamava yeast_bank_events_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_yeast_bank.controller import yeast_bank_events_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(YeastBankEvent)
...
_BOOLEAN_FIELDS = [
    c.name for c in YeastBankEvent.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(YeastBankEvent) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(YeastBankEvent) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(YeastBankEvent) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(YeastBankEvent).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(YeastBankEvent, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(YeastBankEvent)

_LIST_KEY = "yeast_bank_events"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(YeastBankEvent, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(YeastBankEvent, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(YeastBankEvent, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(YeastBankEvent, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(YeastBankEvent, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(YeastBankEvent, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(YeastBankEvent.query.filter(YeastBankEvent.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(YeastBankEvent.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
...
def manage():
    return render_template("yeast_bank_events/manage.html", **_manage_context())


@yeast_bank_events_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("yeast_bank_events.list")
...
@yeast_bank_events_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("yeast_bank_events.list")
def export_csv():
    query = _apply_filters(YeastBankEvent.query.filter(YeastBankEvent.is_deleted.is_(False)))
    items = query.order_by(YeastBankEvent.id.desc()).all()
...
@yeast_bank_events_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("yeast_bank_events.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Evento do Banco"[:31]  # limite do Excel pro nome da aba
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("yeast_bank_events.manage"))
    return render_template("yeast_bank_events/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar YeastBankEvent: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar YeastBankEvent id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@yeast_bank_events_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("yeast_bank_events.trash")
def bulk_trash():
...
@yeast_bank_events_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("yeast_bank_events.update")
def bulk_inactivate():
    from flask import jsonify
