...
# importava/chamava yeast_strains_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_yeast_bank.controller import yeast_strains_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(YeastStrain)
...
for _field, _html_attrs in html_types_for_model(YeastStrain, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(YeastStrain)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(YeastStrain, id)
...
def manage():
    return render_template("yeast_strains/manage.html", **_manage_context())
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("yeast_strains.manage"))
    return render_template("yeast_strains/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar YeastStrain: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar YeastStrain id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@yeast_strains_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("yeast_strains.trash")
def bulk_trash():
...
@yeast_strains_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("yeast_strains.update")
def bulk_inactivate():
    from flask import jsonify
