...
# importava/chamava yeast_cell_count_histories_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_yeast_bank.controller import yeast_cell_count_histories_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(YeastCellCountHistory)
...
_BOOLEAN_FIELDS = [
    c.name for c in YeastCellCountHistory.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(YeastCellCountHistory) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(YeastCellCountHistory) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(YeastCellCountHistory) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(YeastCellCountHistory).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(YeastCellCountHistory, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(YeastCellCountHistory)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(YeastCellCountHistory, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(YeastCellCountHistory, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(YeastCellCountHistory, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(YeastCellCountHistory, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(YeastCellCountHistory, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(YeastCellCountHistory.query.filter(YeastCellCountHistory.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(YeastCellCountHistory.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
...
def manage():
    return render_template("yeast_cell_count_histories/manage.html", **_manage_context())


@yeast_cell_count_histories_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("yeast_cell_count_histories.list")
...
@yeast_cell_count_histories_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("yeast_cell_count_histories.list")
def export_csv():
    query = _apply_filters(YeastCellCountHistory.query.filter(YeastCellCountHistory.is_deleted.is_(False)))
    items = query.order_by(YeastCellCountHistory.id.desc()).all()
...
@yeast_cell_count_histories_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("yeast_cell_count_histories.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Histórico de Contagem"[:31]  # limite do Excel pro nome da aba
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("yeast_cell_count_histories.manage"))
    return render_template("yeast_cell_count_histories/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar YeastCellCountHistory: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar YeastCellCountHistory id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
