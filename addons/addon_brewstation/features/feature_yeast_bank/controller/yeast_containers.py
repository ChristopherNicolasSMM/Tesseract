...
# importava/chamava yeast_containers_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_yeast_bank.controller import yeast_containers_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(YeastContainer)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(YeastContainer, id)
...
@yeast_containers_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("yeast_containers.trash")
def bulk_trash():
...
@yeast_containers_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("yeast_containers.update")
def bulk_inactivate():
    from flask import jsonify
