...
# importava/chamava yeast_storage_devices_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_yeast_bank.controller import yeast_storage_devices_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(YeastStorageDevice)
...
_BOOLEAN_FIELDS = [
    c.name for c in YeastStorageDevice.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(YeastStorageDevice) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(YeastStorageDevice) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(YeastStorageDevice) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(YeastStorageDevice).get("validations", {}).items():
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(YeastStorageDevice, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(YeastStorageDevice, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(YeastStorageDevice, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(YeastStorageDevice, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(YeastStorageDevice, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(YeastStorageDevice, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(YeastStorageDevice.query.filter(YeastStorageDevice.is_deleted.is_(False)))
...
@yeast_storage_devices_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("yeast_storage_devices.list")
def export_csv():
    query = _apply_filters(YeastStorageDevice.query.filter(YeastStorageDevice.is_deleted.is_(False)))
    items = query.order_by(YeastStorageDevice.id.desc()).all()
...
@yeast_storage_devices_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("yeast_storage_devices.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Dispositivo de Armazenamento"[:31]  # limite do Excel pro nome da aba
...
@yeast_storage_devices_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("yeast_storage_devices.trash")
def bulk_trash():
...
@yeast_storage_devices_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("yeast_storage_devices.update")
def bulk_inactivate():
    from flask import jsonify
