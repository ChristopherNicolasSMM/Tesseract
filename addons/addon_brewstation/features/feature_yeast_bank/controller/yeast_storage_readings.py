...
_EDITABLE_FIELDS = [c.name for c in YeastStorageReading.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in YeastStorageReading.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(YeastStorageReading) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(YeastStorageReading) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(YeastStorageReading) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(YeastStorageReading).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(YeastStorageReading, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(YeastStorageReading)

_LIST_KEY = "yeast_storage_readings"
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(YeastStorageReading, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(YeastStorageReading, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(YeastStorageReading, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(YeastStorageReading, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(YeastStorageReading, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(YeastStorageReading.query.filter(YeastStorageReading.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(YeastStorageReading.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Leitura de Armazenamento", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
def manage():
    return render_template("yeast_storage_readings/manage.html", **_manage_context())


@yeast_storage_readings_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("yeast_storage_readings.list")
...
@yeast_storage_readings_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("yeast_storage_readings.list")
def export_csv():
    query = _apply_filters(YeastStorageReading.query.filter(YeastStorageReading.is_deleted.is_(False)))
    items = query.order_by(YeastStorageReading.id.desc()).all()
...
@yeast_storage_readings_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("yeast_storage_readings.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Leitura de Armazenamento"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Leitura de Armazenamento", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("yeast_storage_readings.manage"))
    return render_template("yeast_storage_readings/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar YeastStorageReading: %s", e)
    ...
    if not success:
        ...
        return render_template(
            "yeast_storage_readings/manage.html",
