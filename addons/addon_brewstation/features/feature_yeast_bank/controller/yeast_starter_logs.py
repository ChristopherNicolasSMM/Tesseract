...
# importava/chamava yeast_starter_logs_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_yeast_bank.controller import yeast_starter_logs_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(YeastStarterLog)
_EDITABLE_FIELDS = [c.name for c in YeastStarterLog.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in YeastStarterLog.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(YeastStarterLog) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(YeastStarterLog) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(YeastStarterLog) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(YeastStarterLog).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(YeastStarterLog, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(YeastStarterLog)

_LIST_KEY = "yeast_starter_logs"
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(YeastStarterLog, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(YeastStarterLog, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(YeastStarterLog, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(YeastStarterLog, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(YeastStarterLog, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(YeastStarterLog.query.filter(YeastStarterLog.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(YeastStarterLog.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Starter", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
def manage():
    return render_template("yeast_starter_logs/manage.html", **_manage_context())


@yeast_starter_logs_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("yeast_starter_logs.list")
...
@yeast_starter_logs_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("yeast_starter_logs.list")
def export_csv():
    query = _apply_filters(YeastStarterLog.query.filter(YeastStarterLog.is_deleted.is_(False)))
    items = query.order_by(YeastStarterLog.id.desc()).all()
...
@yeast_starter_logs_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("yeast_starter_logs.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Starter"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Starter", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("yeast_starter_logs.manage"))
    return render_template("yeast_starter_logs/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar YeastStarterLog: %s", e)
    ...
    if not success:
        ...
        return render_template(
            "yeast_starter_logs/manage.html",
