...
_BOOLEAN_FIELDS = [
    c.name for c in YeastBankConfig.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(YeastBankConfig) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(YeastBankConfig) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(YeastBankConfig) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(YeastBankConfig).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(YeastBankConfig, _EDITABLE_FIELDS).items():
...
def _apply_filters(query):
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(YeastBankConfig, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(YeastBankConfig, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(YeastBankConfig, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(YeastBankConfig, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(YeastBankConfig.query.filter(YeastBankConfig.is_deleted.is_(False)))
...
def manage():
    return render_template("yeast_bank_configs/manage.html", **_manage_context())


@yeast_bank_configs_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("yeast_bank_configs.list")
...
@yeast_bank_configs_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("yeast_bank_configs.list")
def export_csv():
    query = _apply_filters(YeastBankConfig.query.filter(YeastBankConfig.is_deleted.is_(False)))
    items = query.order_by(YeastBankConfig.id.desc()).all()
...
@yeast_bank_configs_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("yeast_bank_configs.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Configuração do Banco de Levedura"[:31]  # limite do Excel pro nome da aba
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("yeast_bank_configs.manage"))
    return render_template("yeast_bank_configs/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar YeastBankConfig: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar YeastBankConfig id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
