...
# importava/chamava yeast_bank_items_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_yeast_bank.controller import yeast_bank_items_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(YeastBankItem)
...
for _field, _html_attrs in html_types_for_model(YeastBankItem, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(YeastBankItem)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(YeastBankItem, id)
...
def manage():
    return render_template("yeast_bank_items/manage.html", **_manage_context())
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("yeast_bank_items.manage"))
    return render_template("yeast_bank_items/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar YeastBankItem: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar YeastBankItem id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@yeast_bank_items_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("yeast_bank_items.trash")
def bulk_trash():
...
@yeast_bank_items_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("yeast_bank_items.update")
def bulk_inactivate():
    from flask import jsonify
