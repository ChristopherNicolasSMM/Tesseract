"""
addons/addon_brewstation/features/feature_ingredientes/controller/maltes.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via maltes_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
# importava/chamava maltes_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_ingredientes.controller import maltes_hooks as _hooks
except ImportError:
    _hooks = None
...
maltes_bp = Blueprint(
    "maltes", __name__, url_prefix="/brewstation/maltes"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Malte)
_EDITABLE_FIELDS = [c.name for c in Malte.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in Malte.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(Malte) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(Malte) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(Malte) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(Malte).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(Malte, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(Malte)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(Malte, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(Malte, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(Malte, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(Malte, _ef["field"]) == value)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(Malte.query.filter(Malte.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(Malte.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Malte", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@maltes_bp.route("/", methods=["GET"])
@login_required
@permission_required("maltes.list")
def manage():
    return render_template("maltes/manage.html", **_manage_context())


@maltes_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("maltes.list")
def save_column_prefs():
    ...
    return redirect(url_for("maltes.manage"))


@maltes_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("maltes.list")
def export_csv():
    query = _apply_filters(Malte.query.filter(Malte.is_deleted.is_(False)))
    items = query.order_by(Malte.id.desc()).all()
...
@maltes_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("maltes.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Malte"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Malte", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@maltes_bp.route("/<int:id>", methods=["GET"])
@login_required
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("maltes.manage"))
    return render_template("maltes/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar Malte: %s", e)
...
@maltes_bp.route("/<int:id>", methods=["POST"])
@login_required
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar Malte id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
    ...
    return redirect(url_for("maltes.detail", id=id))


@maltes_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@maltes_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@maltes_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
...
@maltes_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("maltes.trash")
def bulk_trash():
...
@maltes_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("maltes.update")
def bulk_inactivate():
    from flask import jsonify
