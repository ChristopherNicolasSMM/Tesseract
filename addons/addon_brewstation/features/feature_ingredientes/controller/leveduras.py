"""
addons/addon_brewstation/features/feature_ingredientes/controller/leveduras.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via leveduras_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
from addons.addon_brewstation.features.feature_ingredientes.services.levedura_service import LeveduraService
...
# importava/chamava leveduras_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_ingredientes.controller import leveduras_hooks as _hooks
except ImportError:
    _hooks = None
...
leveduras_bp = Blueprint(
    "leveduras", __name__, url_prefix="/brewstation/leveduras"
)
_service = LeveduraService()
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Levedura)
_EDITABLE_FIELDS = [c.name for c in Levedura.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in Levedura.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(Levedura) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(Levedura) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(Levedura) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(Levedura).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(Levedura, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(Levedura)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(Levedura, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(Levedura, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(Levedura, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(Levedura, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(Levedura, _ef["field"]) == value)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(Levedura.query.filter(Levedura.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(Levedura.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Levedura", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@leveduras_bp.route("/", methods=["GET"])
@login_required
@permission_required("leveduras.list")
def manage():
    return render_template("leveduras/manage.html", **_manage_context())


@leveduras_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("leveduras.list")
def save_column_prefs():
    ...
    return redirect(url_for("leveduras.manage"))


@leveduras_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("leveduras.list")
def export_csv():
    query = _apply_filters(Levedura.query.filter(Levedura.is_deleted.is_(False)))
    items = query.order_by(Levedura.id.desc()).all()
...
@leveduras_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("leveduras.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Levedura"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Levedura", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@leveduras_bp.route("/<int:id>", methods=["GET"])
@login_required
@permission_required("leveduras.detail")
def detail(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
        return redirect(url_for("leveduras.manage"))
    return render_template("leveduras/detail.html", **_detail_context(item))
...
@leveduras_bp.route("/", methods=["POST"])
@login_required
@permission_required("leveduras.create")
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar Levedura: %s", e)
...
@leveduras_bp.route("/<int:id>", methods=["POST"])
@login_required
@permission_required("leveduras.update")
def update(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar Levedura id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "leveduras/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("leveduras.detail", id=id))


@leveduras_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("leveduras.trash")
def trash(id: int):
...
@leveduras_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("leveduras.restore")
def restore(id: int):
...
@leveduras_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
@permission_required("leveduras.delete_permanent")
...
@leveduras_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("leveduras.trash")
def bulk_trash():
...
@leveduras_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("leveduras.update")
def bulk_inactivate():
    from flask import jsonify
