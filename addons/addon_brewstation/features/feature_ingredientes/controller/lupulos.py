"""
addons/addon_brewstation/features/feature_ingredientes/controller/lupulos.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via lupulos_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
# importava/chamava lupulos_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_ingredientes.controller import lupulos_hooks as _hooks
except ImportError:
    _hooks = None
...
lupulos_bp = Blueprint(
    "lupulos", __name__, url_prefix="/brewstation/lupulos"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Lupulo)
_EDITABLE_FIELDS = [c.name for c in Lupulo.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in Lupulo.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(Lupulo) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(Lupulo) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(Lupulo) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(Lupulo).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(Lupulo, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(Lupulo)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(Lupulo, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(Lupulo, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(Lupulo, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(Lupulo, _ef["field"]) == value)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(Lupulo.query.filter(Lupulo.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(Lupulo.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Lúpulo", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@lupulos_bp.route("/", methods=["GET"])
@login_required
@permission_required("lupulos.list")
def manage():
    return render_template("lupulos/manage.html", **_manage_context())


@lupulos_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("lupulos.list")
def save_column_prefs():
    ...
    return redirect(url_for("lupulos.manage"))


@lupulos_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("lupulos.list")
def export_csv():
    query = _apply_filters(Lupulo.query.filter(Lupulo.is_deleted.is_(False)))
    items = query.order_by(Lupulo.id.desc()).all()
...
@lupulos_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("lupulos.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Lúpulo"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Lúpulo", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@lupulos_bp.route("/<int:id>", methods=["GET"])
@login_required
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("lupulos.manage"))
    return render_template("lupulos/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar Lupulo: %s", e)
...
@lupulos_bp.route("/<int:id>", methods=["POST"])
@login_required
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar Lupulo id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
    ...
    return redirect(url_for("lupulos.detail", id=id))


@lupulos_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@lupulos_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@lupulos_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
...
@lupulos_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("lupulos.trash")
def bulk_trash():
...
@lupulos_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("lupulos.update")
def bulk_inactivate():
    from flask import jsonify
