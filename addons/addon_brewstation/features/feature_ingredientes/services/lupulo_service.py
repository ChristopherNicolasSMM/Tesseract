"""
addons/addon_brewstation/features/feature_ingredientes/services/lupulo_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(lupulo_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Lupulo)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in Lupulo.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_ingredientes.services import lupulo_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class LupuloService:
    """Camada de negócio para Lúpulo."""

    def list(self, *, include_deleted: bool = False):
        query = Lupulo.query
        if not include_deleted:
            query = query.filter(Lupulo.is_deleted.is_(False))
        return query.order_by(Lupulo.id.asc()).all()

    def get_by_id(self, id: int) -> "Lupulo | None":
        return db.session.get(Lupulo, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Lupulo: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Lupulo id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
