"""
addons/addon_brewstation/features/feature_ingredientes/services/levedura_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(levedura_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Levedura)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in Levedura.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_ingredientes.services import levedura_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class LeveduraService:
    """Camada de negócio para Levedura."""

    def list(self, *, include_deleted: bool = False):
        query = Levedura.query
        if not include_deleted:
            query = query.filter(Levedura.is_deleted.is_(False))
        return query.order_by(Levedura.id.asc()).all()

    def get_by_id(self, id: int) -> "Levedura | None":
        return db.session.get(Levedura, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Levedura: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Levedura id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
