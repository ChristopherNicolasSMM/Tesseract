"""
addons/addon_brewstation/features/feature_ingredientes/services/malte_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(malte_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(Malte)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in Malte.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_ingredientes.services import malte_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class MalteService:
    """Camada de negócio para Malte."""

    def list(self, *, include_deleted: bool = False):
        query = Malte.query
        if not include_deleted:
            query = query.filter(Malte.is_deleted.is_(False))
        return query.order_by(Malte.id.asc()).all()

    def get_by_id(self, id: int) -> "Malte | None":
        return db.session.get(Malte, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar Malte: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar Malte id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
