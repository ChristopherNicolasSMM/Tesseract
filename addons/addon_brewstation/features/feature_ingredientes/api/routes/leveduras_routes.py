"""
addons/addon_brewstation/features/feature_ingredientes/api/routes/leveduras_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via leveduras_routes_hooks.py (nunca sobrescrito).
"""
...
leveduras_api_bp = Blueprint(
    "leveduras_api", __name__, url_prefix="/api/brewstation/leveduras"
)
...
@leveduras_api_bp.route("/", methods=["GET"])
@login_required
...
@leveduras_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@leveduras_api_bp.route("/", methods=["POST"])
@login_required
...
@leveduras_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@leveduras_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@leveduras_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@leveduras_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
