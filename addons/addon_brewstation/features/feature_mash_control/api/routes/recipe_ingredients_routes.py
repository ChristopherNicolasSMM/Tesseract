"""
addons/addon_brewstation/features/feature_mash_control/api/routes/recipe_ingredients_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via recipe_ingredients_routes_hooks.py (nunca sobrescrito).
"""
...
recipe_ingredients_api_bp = Blueprint(
    "recipe_ingredients_api", __name__, url_prefix="/api/brewstation/recipe-ingredients"
)
...
@recipe_ingredients_api_bp.route("/", methods=["GET"])
@login_required
...
@recipe_ingredients_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@recipe_ingredients_api_bp.route("/", methods=["POST"])
@login_required
...
@recipe_ingredients_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@recipe_ingredients_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@recipe_ingredients_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@recipe_ingredients_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
