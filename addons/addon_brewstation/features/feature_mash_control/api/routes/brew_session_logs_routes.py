"""
addons/addon_brewstation/features/feature_mash_control/api/routes/brew_session_logs_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via brew_session_logs_routes_hooks.py (nunca sobrescrito).
"""
...
brew_session_logs_api_bp = Blueprint(
    "brew_session_logs_api", __name__, url_prefix="/api/brewstation/brew-session-logs"
)
...
@brew_session_logs_api_bp.route("/", methods=["GET"])
@login_required
...
@brew_session_logs_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@brew_session_logs_api_bp.route("/", methods=["POST"])
@login_required
...
@brew_session_logs_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@brew_session_logs_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@brew_session_logs_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@brew_session_logs_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
