"""
addons/addon_brewstation/features/feature_mash_control/api/routes/ingredient_mappings_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via ingredient_mappings_routes_hooks.py (nunca sobrescrito).
"""
...
ingredient_mappings_api_bp = Blueprint(
    "ingredient_mappings_api", __name__, url_prefix="/api/brewstation/ingredient-mappings"
)
...
@ingredient_mappings_api_bp.route("/", methods=["GET"])
@login_required
...
@ingredient_mappings_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@ingredient_mappings_api_bp.route("/", methods=["POST"])
@login_required
...
@ingredient_mappings_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@ingredient_mappings_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@ingredient_mappings_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@ingredient_mappings_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
