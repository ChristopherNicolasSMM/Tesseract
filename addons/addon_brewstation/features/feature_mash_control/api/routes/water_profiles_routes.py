"""
addons/addon_brewstation/features/feature_mash_control/api/routes/water_profiles_routes.py

API JSON — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via water_profiles_routes_hooks.py (nunca sobrescrito).
"""
...
water_profiles_api_bp = Blueprint(
    "water_profiles_api", __name__, url_prefix="/api/brewstation/water-profiles"
)
...
@water_profiles_api_bp.route("/", methods=["GET"])
@login_required
...
@water_profiles_api_bp.route("/<int:id>", methods=["GET"])
@login_required
...
@water_profiles_api_bp.route("/", methods=["POST"])
@login_required
...
@water_profiles_api_bp.route("/<int:id>", methods=["PUT"])
@login_required
...
@water_profiles_api_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@water_profiles_api_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@water_profiles_api_bp.route("/<int:id>", methods=["DELETE"])
@login_required
