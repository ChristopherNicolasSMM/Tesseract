...
# importava/chamava brew_session_steps_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_mash_control.controller import brew_session_steps_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(BrewSessionStep)
...
_BOOLEAN_FIELDS = [
    c.name for c in BrewSessionStep.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(BrewSessionStep) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(BrewSessionStep) if wr["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(BrewSessionStep).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(BrewSessionStep, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(BrewSessionStep)

_LIST_KEY = "brew_session_steps"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(BrewSessionStep, id)
...
def _apply_filters(query):
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(BrewSessionStep, field).is_(value == "true"))
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(BrewSessionStep, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(BrewSessionStep.query.filter(BrewSessionStep.is_deleted.is_(False)))
...
def manage():
    return render_template("brew_session_steps/manage.html", **_manage_context())


@brew_session_steps_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("brew_session_steps.list")
...
@brew_session_steps_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("brew_session_steps.list")
def export_csv():
    query = _apply_filters(BrewSessionStep.query.filter(BrewSessionStep.is_deleted.is_(False)))
    items = query.order_by(BrewSessionStep.id.desc()).all()
...
@brew_session_steps_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("brew_session_steps.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Passo da Sessão"[:31]  # limite do Excel pro nome da aba
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("brew_session_steps.manage"))
    return render_template("brew_session_steps/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar BrewSessionStep: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar BrewSessionStep id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@brew_session_steps_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("brew_session_steps.trash")
def bulk_trash():
...
@brew_session_steps_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("brew_session_steps.update")
def bulk_inactivate():
    from flask import jsonify
