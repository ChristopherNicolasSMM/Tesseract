"""
addons/addon_brewstation/features/feature_mash_control/controller/fermentation_steps.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via fermentation_steps_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
from addons.addon_brewstation.features.feature_mash_control.services.fermentation_step_service import FermentationStepService
...
# importava/chamava fermentation_steps_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_mash_control.controller import fermentation_steps_hooks as _hooks
except ImportError:
    _hooks = None
...
fermentation_steps_bp = Blueprint(
    "fermentation_steps", __name__, url_prefix="/brewstation/fermentation-steps"
)
_service = FermentationStepService()
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(FermentationStep)
_EDITABLE_FIELDS = [c.name for c in FermentationStep.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in FermentationStep.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(FermentationStep) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(FermentationStep) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(FermentationStep) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(FermentationStep).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(FermentationStep, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(FermentationStep)

_LIST_KEY = "fermentation_steps"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(FermentationStep, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(FermentationStep, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(FermentationStep, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(FermentationStep, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(FermentationStep, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(FermentationStep, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(FermentationStep.query.filter(FermentationStep.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(FermentationStep.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Etapa de Fermentação", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@fermentation_steps_bp.route("/", methods=["GET"])
@login_required
@permission_required("fermentation_steps.list")
def manage():
    return render_template("fermentation_steps/manage.html", **_manage_context())


@fermentation_steps_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("fermentation_steps.list")
def save_column_prefs():
    ...
    return redirect(url_for("fermentation_steps.manage"))


@fermentation_steps_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("fermentation_steps.list")
def export_csv():
    query = _apply_filters(FermentationStep.query.filter(FermentationStep.is_deleted.is_(False)))
    items = query.order_by(FermentationStep.id.desc()).all()
...
@fermentation_steps_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("fermentation_steps.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Etapa de Fermentação"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Etapa de Fermentação", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@fermentation_steps_bp.route("/<int:id>", methods=["GET"])
@login_required
@permission_required("fermentation_steps.detail")
def detail(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
        return redirect(url_for("fermentation_steps.manage"))
    return render_template("fermentation_steps/detail.html", **_detail_context(item))
...
@fermentation_steps_bp.route("/", methods=["POST"])
@login_required
@permission_required("fermentation_steps.create")
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar FermentationStep: %s", e)
    ...
    if not success:
        ...
        return render_template(
            "fermentation_steps/manage.html",
...
@fermentation_steps_bp.route("/<int:id>", methods=["POST"])
@login_required
@permission_required("fermentation_steps.update")
def update(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar FermentationStep id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "fermentation_steps/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("fermentation_steps.detail", id=id))


@fermentation_steps_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("fermentation_steps.trash")
def trash(id: int):
...
@fermentation_steps_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("fermentation_steps.restore")
def restore(id: int):
...
@fermentation_steps_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
@permission_required("fermentation_steps.delete_permanent")
...
@fermentation_steps_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("fermentation_steps.trash")
def bulk_trash():
...
@fermentation_steps_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("fermentation_steps.update")
def bulk_inactivate():
    from flask import jsonify
