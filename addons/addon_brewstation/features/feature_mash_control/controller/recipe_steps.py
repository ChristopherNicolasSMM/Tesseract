...
# importava/chamava recipe_steps_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_mash_control.controller import recipe_steps_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(RecipeStep)
...
for _field, _html_attrs in html_types_for_model(RecipeStep, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(RecipeStep)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(RecipeStep, id)
...
def manage():
    return render_template("recipe_steps/manage.html", **_manage_context())
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("recipe_steps.manage"))
    return render_template("recipe_steps/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar RecipeStep: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar RecipeStep id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@recipe_steps_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("recipe_steps.trash")
def bulk_trash():
...
@recipe_steps_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("recipe_steps.update")
def bulk_inactivate():
    from flask import jsonify
