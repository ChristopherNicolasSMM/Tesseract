"""
addons/addon_brewstation/features/feature_mash_control/controller/mash_steps.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via mash_steps_hooks.py (nunca sobrescrito).
"""
import csv
import io
...
mash_steps_bp = Blueprint(
    "mash_steps", __name__, url_prefix="/brewstation/mash-steps"
)
...
_EDITABLE_FIELDS = [c.name for c in MashStep.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in MashStep.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(MashStep) if f["field"] in _EDITABLE_FIELDS]

_LIST_KEY = "mash_steps"
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(MashStep, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(MashStep, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(MashStep, field) == value)
...
def manage():
    ...
    query = _apply_filters(MashStep.query.filter(MashStep.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(MashStep.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return render_template(
        "mash_steps/manage.html",
        items=items, label="Passo de Mostura", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@mash_steps_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("mash_steps.list")
def save_column_prefs():
    ...
    return redirect(url_for("mash_steps.manage"))


@mash_steps_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("mash_steps.list")
def export_csv():
    query = _apply_filters(MashStep.query.filter(MashStep.is_deleted.is_(False)))
    items = query.order_by(MashStep.id.desc()).all()
...
@mash_steps_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("mash_steps.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Passo de Mostura"[:31]  # limite do Excel pro nome da aba
...
@mash_steps_bp.route("/<int:id>", methods=["GET"])
@login_required
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("mash_steps.manage"))
    return render_template(
        "mash_steps/detail.html",
        item=item, label="Passo de Mostura", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
    )


@mash_steps_bp.route("/", methods=["POST"])
@login_required
...
@mash_steps_bp.route("/<int:id>", methods=["POST"])
@login_required
...
def update(id: int):
    ...
    return redirect(url_for("mash_steps.detail", id=id))


@mash_steps_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@mash_steps_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@mash_steps_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
