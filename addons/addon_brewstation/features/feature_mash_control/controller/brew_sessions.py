...
try:
    from addons.addon_brewstation.features.feature_mash_control.controller import brew_sessions_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(BrewSession)
...
_BOOLEAN_FIELDS = [
    c.name for c in BrewSession.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(BrewSession) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(BrewSession) if wr["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(BrewSession).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(BrewSession, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(BrewSession)

_LIST_KEY = "brew_sessions"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(BrewSession, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(BrewSession, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(BrewSession, field).is_(value == "true"))
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(BrewSession, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(BrewSession.query.filter(BrewSession.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(BrewSession.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
...
def manage():
    return render_template("brew_sessions/manage.html", **_manage_context())


@brew_sessions_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("brew_sessions.list")
...
@brew_sessions_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("brew_sessions.list")
def export_csv():
    query = _apply_filters(BrewSession.query.filter(BrewSession.is_deleted.is_(False)))
    items = query.order_by(BrewSession.id.desc()).all()
...
@brew_sessions_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("brew_sessions.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Sessão de Brassagem"[:31]  # limite do Excel pro nome da aba
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("brew_sessions.manage"))
    return render_template("brew_sessions/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar BrewSession: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar BrewSession id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@brew_sessions_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("brew_sessions.trash")
def bulk_trash():
...
@brew_sessions_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("brew_sessions.update")
def bulk_inactivate():
    from flask import jsonify
