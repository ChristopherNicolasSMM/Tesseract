...
# importava/chamava brew_session_alarms_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_mash_control.controller import brew_session_alarms_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(BrewSessionAlarm)
...
_BOOLEAN_FIELDS = [
    c.name for c in BrewSessionAlarm.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(BrewSessionAlarm) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(BrewSessionAlarm) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(BrewSessionAlarm) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(BrewSessionAlarm).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(BrewSessionAlarm, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(BrewSessionAlarm)

_LIST_KEY = "brew_session_alarms"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(BrewSessionAlarm, id)
...
def _apply_filters(query):
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(BrewSessionAlarm, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(BrewSessionAlarm, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(BrewSessionAlarm, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(BrewSessionAlarm, field)
...
def manage():
    return render_template("brew_session_alarms/manage.html", **_manage_context())


@brew_session_alarms_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("brew_session_alarms.list")
...
@brew_session_alarms_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("brew_session_alarms.list")
def export_csv():
    query = _apply_filters(BrewSessionAlarm.query.filter(BrewSessionAlarm.is_deleted.is_(False)))
    items = query.order_by(BrewSessionAlarm.id.desc()).all()
...
@brew_session_alarms_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("brew_session_alarms.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Alarme da Sessão"[:31]  # limite do Excel pro nome da aba
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("brew_session_alarms.manage"))
    return render_template("brew_session_alarms/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar BrewSessionAlarm: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar BrewSessionAlarm id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@brew_session_alarms_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("brew_session_alarms.trash")
def bulk_trash():
...
@brew_session_alarms_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("brew_session_alarms.update")
def bulk_inactivate():
    from flask import jsonify
