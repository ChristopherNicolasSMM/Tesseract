"""
addons/addon_brewstation/features/feature_mash_control/controller/recipe_historys.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via recipe_historys_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
# importava/chamava recipe_historys_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_mash_control.controller import recipe_historys_hooks as _hooks
except ImportError:
    _hooks = None
...
recipe_historys_bp = Blueprint(
    "recipe_historys", __name__, url_prefix="/brewstation/recipe-historys"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(RecipeHistory)
_EDITABLE_FIELDS = [c.name for c in RecipeHistory.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in RecipeHistory.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(RecipeHistory) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(RecipeHistory) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(RecipeHistory) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(RecipeHistory).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(RecipeHistory, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(RecipeHistory)

_LIST_KEY = "recipe_historys"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(RecipeHistory, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(RecipeHistory, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(RecipeHistory, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(RecipeHistory, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(RecipeHistory, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(RecipeHistory, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(RecipeHistory.query.filter(RecipeHistory.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(RecipeHistory.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Histórico de Receita", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@recipe_historys_bp.route("/", methods=["GET"])
@login_required
@permission_required("recipe_historys.list")
def manage():
    return render_template("recipe_historys/manage.html", **_manage_context())


@recipe_historys_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("recipe_historys.list")
def save_column_prefs():
    ...
    return redirect(url_for("recipe_historys.manage"))


@recipe_historys_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("recipe_historys.list")
def export_csv():
    query = _apply_filters(RecipeHistory.query.filter(RecipeHistory.is_deleted.is_(False)))
    items = query.order_by(RecipeHistory.id.desc()).all()
...
@recipe_historys_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("recipe_historys.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Histórico de Receita"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Histórico de Receita", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@recipe_historys_bp.route("/<int:id>", methods=["GET"])
@login_required
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("recipe_historys.manage"))
    return render_template("recipe_historys/detail.html", **_detail_context(item))
...
@recipe_historys_bp.route("/", methods=["POST"])
@login_required
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar RecipeHistory: %s", e)
...
@recipe_historys_bp.route("/<int:id>", methods=["POST"])
@login_required
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar RecipeHistory id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "recipe_historys/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("recipe_historys.detail", id=id))


@recipe_historys_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@recipe_historys_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@recipe_historys_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
...
@recipe_historys_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("recipe_historys.trash")
def bulk_trash():
...
@recipe_historys_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("recipe_historys.update")
def bulk_inactivate():
    from flask import jsonify
