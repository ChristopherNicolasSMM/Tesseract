"""
addons/addon_brewstation/features/feature_mash_control/controller/recipe_ingredients.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via recipe_ingredients_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
from addons.addon_brewstation.features.feature_mash_control.services.recipe_ingredient_service import RecipeIngredientService
...
# importava/chamava recipe_ingredients_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_mash_control.controller import recipe_ingredients_hooks as _hooks
except ImportError:
    _hooks = None
...
recipe_ingredients_bp = Blueprint(
    "recipe_ingredients", __name__, url_prefix="/brewstation/recipe-ingredients"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(RecipeIngredient)
_EDITABLE_FIELDS = [c.name for c in RecipeIngredient.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in RecipeIngredient.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(RecipeIngredient) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(RecipeIngredient) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(RecipeIngredient) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(RecipeIngredient).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(RecipeIngredient, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(RecipeIngredient)

_LIST_KEY = "recipe_ingredients"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(RecipeIngredient, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(RecipeIngredient, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(RecipeIngredient, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(RecipeIngredient, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(RecipeIngredient, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(RecipeIngredient, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(RecipeIngredient.query.filter(RecipeIngredient.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(RecipeIngredient.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Ingrediente de Receita", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
def manage():
    return render_template("recipe_ingredients/manage.html", **_manage_context())


@recipe_ingredients_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("recipe_ingredients.list")
def save_column_prefs():
    ...
    return redirect(url_for("recipe_ingredients.manage"))


@recipe_ingredients_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("recipe_ingredients.list")
def export_csv():
    query = _apply_filters(RecipeIngredient.query.filter(RecipeIngredient.is_deleted.is_(False)))
    items = query.order_by(RecipeIngredient.id.desc()).all()
...
@recipe_ingredients_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("recipe_ingredients.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Ingrediente de Receita"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Ingrediente de Receita", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@login_required
@permission_required("recipe_ingredients.detail")
def detail(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
        return redirect(url_for("recipe_ingredients.manage"))
    return render_template("recipe_ingredients/detail.html", **_detail_context(item))
...
@recipe_ingredients_bp.route("/", methods=["POST"])
@login_required
@permission_required("recipe_ingredients.create")
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar RecipeIngredient: %s", e)
    ...
    if not success:
        ...
        return render_template(
            "recipe_ingredients/manage.html",
...
@recipe_ingredients_bp.route("/<int:id>", methods=["POST"])
@login_required
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar RecipeIngredient id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "recipe_ingredients/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("recipe_ingredients.detail", id=id))


@recipe_ingredients_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("recipe_ingredients.trash")
def trash(id: int):
...
@recipe_ingredients_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("recipe_ingredients.restore")
def restore(id: int):
...
@recipe_ingredients_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
@permission_required("recipe_ingredients.delete_permanent")
...
@recipe_ingredients_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("recipe_ingredients.trash")
def bulk_trash():
...
@recipe_ingredients_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("recipe_ingredients.update")
def bulk_inactivate():
    from flask import jsonify
