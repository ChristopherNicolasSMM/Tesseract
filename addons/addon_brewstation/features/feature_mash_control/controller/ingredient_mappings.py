"""
addons/addon_brewstation/features/feature_mash_control/controller/ingredient_mappings.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via ingredient_mappings_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
from addons.addon_brewstation.features.feature_mash_control.services.ingredient_mapping_service import IngredientMappingService
...
# importava/chamava ingredient_mappings_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_mash_control.controller import ingredient_mappings_hooks as _hooks
except ImportError:
    _hooks = None
...
ingredient_mappings_bp = Blueprint(
    "ingredient_mappings", __name__, url_prefix="/brewstation/ingredient-mappings"
)
_service = IngredientMappingService()
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(IngredientMapping)
_EDITABLE_FIELDS = [c.name for c in IngredientMapping.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in IngredientMapping.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(IngredientMapping) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(IngredientMapping) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(IngredientMapping) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(IngredientMapping).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(IngredientMapping, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(IngredientMapping)

_LIST_KEY = "ingredient_mappings"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(IngredientMapping, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(IngredientMapping, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(IngredientMapping, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(IngredientMapping, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(IngredientMapping, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(IngredientMapping, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(IngredientMapping.query.filter(IngredientMapping.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(IngredientMapping.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Mapeamento de Ingrediente", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@ingredient_mappings_bp.route("/", methods=["GET"])
@login_required
@permission_required("ingredient_mappings.list")
def manage():
    return render_template("ingredient_mappings/manage.html", **_manage_context())


@ingredient_mappings_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("ingredient_mappings.list")
def save_column_prefs():
    ...
    return redirect(url_for("ingredient_mappings.manage"))


@ingredient_mappings_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("ingredient_mappings.list")
def export_csv():
    query = _apply_filters(IngredientMapping.query.filter(IngredientMapping.is_deleted.is_(False)))
    items = query.order_by(IngredientMapping.id.desc()).all()
...
@ingredient_mappings_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("ingredient_mappings.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Mapeamento de Ingrediente"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Mapeamento de Ingrediente", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@ingredient_mappings_bp.route("/<int:id>", methods=["GET"])
@login_required
@permission_required("ingredient_mappings.detail")
def detail(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
        return redirect(url_for("ingredient_mappings.manage"))
    return render_template("ingredient_mappings/detail.html", **_detail_context(item))
...
@ingredient_mappings_bp.route("/", methods=["POST"])
@login_required
@permission_required("ingredient_mappings.create")
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar IngredientMapping: %s", e)
    ...
    if not success:
        ...
        return render_template(
            "ingredient_mappings/manage.html",
...
@ingredient_mappings_bp.route("/<int:id>", methods=["POST"])
@login_required
@permission_required("ingredient_mappings.update")
def update(id: int):
    item = _service.get_by_id(id)
    if not item:
        flash("Registro não encontrado.", "error")
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar IngredientMapping id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "ingredient_mappings/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("ingredient_mappings.detail", id=id))


@ingredient_mappings_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
@permission_required("ingredient_mappings.trash")
def trash(id: int):
...
@ingredient_mappings_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
@permission_required("ingredient_mappings.restore")
def restore(id: int):
...
@ingredient_mappings_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
@permission_required("ingredient_mappings.delete_permanent")
...
@ingredient_mappings_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("ingredient_mappings.trash")
def bulk_trash():
...
@ingredient_mappings_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("ingredient_mappings.update")
def bulk_inactivate():
    from flask import jsonify
