...
# importava/chamava brew_plant_mappings_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_mash_control.controller import brew_plant_mappings_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(BrewPlantMapping)
_EDITABLE_FIELDS = [c.name for c in BrewPlantMapping.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in BrewPlantMapping.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(BrewPlantMapping) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(BrewPlantMapping) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(BrewPlantMapping) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(BrewPlantMapping).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(BrewPlantMapping, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(BrewPlantMapping)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(BrewPlantMapping, id)
...
def _apply_filters(query):
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(BrewPlantMapping, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(BrewPlantMapping, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(BrewPlantMapping, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(BrewPlantMapping, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(BrewPlantMapping.query.filter(BrewPlantMapping.is_deleted.is_(False)))
...
def manage():
    return render_template("brew_plant_mappings/manage.html", **_manage_context())


@brew_plant_mappings_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("brew_plant_mappings.list")
...
@brew_plant_mappings_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("brew_plant_mappings.list")
def export_csv():
    query = _apply_filters(BrewPlantMapping.query.filter(BrewPlantMapping.is_deleted.is_(False)))
    items = query.order_by(BrewPlantMapping.id.desc()).all()
...
@brew_plant_mappings_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("brew_plant_mappings.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Mapeamento de Equipamento"[:31]  # limite do Excel pro nome da aba
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("brew_plant_mappings.manage"))
    return render_template("brew_plant_mappings/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar BrewPlantMapping: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar BrewPlantMapping id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
    ...
    return redirect(url_for("brew_plant_mappings.detail", id=id))
...
@brew_plant_mappings_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("brew_plant_mappings.trash")
def bulk_trash():
...
@brew_plant_mappings_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("brew_plant_mappings.update")
def bulk_inactivate():
    from flask import jsonify
