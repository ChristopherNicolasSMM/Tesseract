...
# importava/chamava mash_recipes_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_mash_control.controller import mash_recipes_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(MashRecipe)
...
_BOOLEAN_FIELDS = [
    c.name for c in MashRecipe.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(MashRecipe) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(MashRecipe) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(MashRecipe) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(MashRecipe).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(MashRecipe, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(MashRecipe)

_LIST_KEY = "mash_recipes"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(MashRecipe, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(MashRecipe, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(MashRecipe, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(MashRecipe, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(MashRecipe, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(MashRecipe, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(MashRecipe.query.filter(MashRecipe.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(MashRecipe.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
...
def manage():
    return render_template("mash_recipes/manage.html", **_manage_context())


@mash_recipes_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("mash_recipes.list")
...
@mash_recipes_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("mash_recipes.list")
def export_csv():
    query = _apply_filters(MashRecipe.query.filter(MashRecipe.is_deleted.is_(False)))
    items = query.order_by(MashRecipe.id.desc()).all()
...
@mash_recipes_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("mash_recipes.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Receita de Brassagem"[:31]  # limite do Excel pro nome da aba
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("mash_recipes.manage"))
    return render_template("mash_recipes/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar MashRecipe: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar MashRecipe id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@mash_recipes_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("mash_recipes.trash")
def bulk_trash():
...
@mash_recipes_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("mash_recipes.update")
def bulk_inactivate():
    from flask import jsonify
