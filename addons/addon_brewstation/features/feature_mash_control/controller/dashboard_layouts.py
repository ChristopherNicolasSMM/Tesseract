...
# importava/chamava dashboard_layouts_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_mash_control.controller import dashboard_layouts_hooks as _hooks
except ImportError:
    _hooks = None
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(DashboardLayout)
...
_WEAK_REFS = [wr for wr in get_weak_refs(DashboardLayout) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(DashboardLayout) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(DashboardLayout).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(DashboardLayout, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(DashboardLayout)
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(DashboardLayout, id)
...
def _apply_filters(query):
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(DashboardLayout, _ef["field"]) == value)
...
def manage():
    return render_template("dashboard_layouts/manage.html", **_manage_context())
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("dashboard_layouts.manage"))
    return render_template("dashboard_layouts/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar DashboardLayout: %s", e)
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar DashboardLayout id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
...
@dashboard_layouts_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("dashboard_layouts.trash")
def bulk_trash():
...
@dashboard_layouts_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("dashboard_layouts.update")
def bulk_inactivate():
    from flask import jsonify
