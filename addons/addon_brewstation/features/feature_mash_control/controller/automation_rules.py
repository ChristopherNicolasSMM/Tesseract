...
# importava/chamava automation_rules_hooks.py de verdade, só tinha o
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(AutomationRule)
_EDITABLE_FIELDS = [c.name for c in AutomationRule.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in AutomationRule.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(AutomationRule) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(AutomationRule) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(AutomationRule) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(AutomationRule).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(AutomationRule, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(AutomationRule)

_LIST_KEY = "automation_rules"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(AutomationRule, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(AutomationRule, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(AutomationRule, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(AutomationRule, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(AutomationRule, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(AutomationRule, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(AutomationRule.query.filter(AutomationRule.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(AutomationRule.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Regra de Automação", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
def manage():
    return render_template("automation_rules/manage.html", **_manage_context())


@automation_rules_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("automation_rules.list")
...
@automation_rules_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("automation_rules.list")
def export_csv():
    query = _apply_filters(AutomationRule.query.filter(AutomationRule.is_deleted.is_(False)))
    items = query.order_by(AutomationRule.id.desc()).all()
...
@automation_rules_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("automation_rules.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Regra de Automação"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Regra de Automação", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("automation_rules.manage"))
    return render_template("automation_rules/detail.html", **_detail_context(item))
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar AutomationRule: %s", e)
...
@automation_rules_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("automation_rules.trash")
def bulk_trash():
...
@automation_rules_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("automation_rules.update")
def bulk_inactivate():
    from flask import jsonify
