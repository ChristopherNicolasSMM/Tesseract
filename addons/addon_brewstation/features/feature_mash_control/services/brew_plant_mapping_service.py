"""
addons/addon_brewstation/features/feature_mash_control/services/brew_plant_mapping_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(brew_plant_mapping_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(BrewPlantMapping)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in BrewPlantMapping.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import brew_plant_mapping_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class BrewPlantMappingService:
    """Camada de negócio para Mapeamento de Equipamento."""

    def list(self, *, include_deleted: bool = False):
        query = BrewPlantMapping.query
        if not include_deleted:
            query = query.filter(BrewPlantMapping.is_deleted.is_(False))
        return query.order_by(BrewPlantMapping.id.asc()).all()

    def get_by_id(self, id: int) -> "BrewPlantMapping | None":
        return db.session.get(BrewPlantMapping, id)

    def create(self, data: dict) -> ServiceResult:
        obj = BrewPlantMapping()
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar BrewPlantMapping: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar BrewPlantMapping id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
