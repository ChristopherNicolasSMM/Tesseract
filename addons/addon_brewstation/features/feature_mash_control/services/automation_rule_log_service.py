"""
addons/addon_brewstation/features/feature_mash_control/services/automation_rule_log_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(automation_rule_log_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(AutomationRuleLog)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in AutomationRuleLog.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import automation_rule_log_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class AutomationRuleLogService:
    """Camada de negócio para Log de Automação."""

    def list(self, *, include_deleted: bool = False):
        query = AutomationRuleLog.query
        if not include_deleted:
            query = query.filter(AutomationRuleLog.is_deleted.is_(False))
        return query.order_by(AutomationRuleLog.id.asc()).all()

    def get_by_id(self, id: int) -> "AutomationRuleLog | None":
        return db.session.get(AutomationRuleLog, id)

    def create(self, data: dict) -> ServiceResult:
        obj = AutomationRuleLog()
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar AutomationRuleLog: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar AutomationRuleLog id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
