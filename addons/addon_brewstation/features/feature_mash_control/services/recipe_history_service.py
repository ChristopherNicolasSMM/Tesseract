"""
addons/addon_brewstation/features/feature_mash_control/services/recipe_history_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(recipe_history_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(RecipeHistory)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in RecipeHistory.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import recipe_history_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class RecipeHistoryService:
    """Camada de negócio para Histórico de Receita."""

    def list(self, *, include_deleted: bool = False):
        query = RecipeHistory.query
        if not include_deleted:
            query = query.filter(RecipeHistory.is_deleted.is_(False))
        return query.order_by(RecipeHistory.id.asc()).all()

    def get_by_id(self, id: int) -> "RecipeHistory | None":
        return db.session.get(RecipeHistory, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar RecipeHistory: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar RecipeHistory id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
