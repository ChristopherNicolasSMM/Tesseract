...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(RecipeStep)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in RecipeStep.__table__.columns.keys()),
    None,
)
