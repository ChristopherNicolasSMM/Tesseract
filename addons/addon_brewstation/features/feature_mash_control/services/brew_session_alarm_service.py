"""
addons/addon_brewstation/features/feature_mash_control/services/brew_session_alarm_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(brew_session_alarm_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(BrewSessionAlarm)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in BrewSessionAlarm.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import brew_session_alarm_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class BrewSessionAlarmService:
    """Camada de negócio para Alarme da Sessão."""

    def list(self, *, include_deleted: bool = False):
        query = BrewSessionAlarm.query
        if not include_deleted:
            query = query.filter(BrewSessionAlarm.is_deleted.is_(False))
        return query.order_by(BrewSessionAlarm.id.asc()).all()

    def get_by_id(self, id: int) -> "BrewSessionAlarm | None":
        return db.session.get(BrewSessionAlarm, id)

    def create(self, data: dict) -> ServiceResult:
        obj = BrewSessionAlarm()
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar BrewSessionAlarm: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar BrewSessionAlarm id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
