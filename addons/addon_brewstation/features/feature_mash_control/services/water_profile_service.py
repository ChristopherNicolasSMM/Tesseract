"""
addons/addon_brewstation/features/feature_mash_control/services/water_profile_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(water_profile_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(WaterProfile)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in WaterProfile.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import water_profile_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class WaterProfileService:
    """Camada de negócio para Perfil de Água."""

    def list(self, *, include_deleted: bool = False):
        query = WaterProfile.query
        if not include_deleted:
            query = query.filter(WaterProfile.is_deleted.is_(False))
        return query.order_by(WaterProfile.id.asc()).all()

    def get_by_id(self, id: int) -> "WaterProfile | None":
        return db.session.get(WaterProfile, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar WaterProfile: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar WaterProfile id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
