"""
addons/addon_brewstation/features/feature_mash_control/services/ingredient_mapping_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(ingredient_mapping_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(IngredientMapping)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in IngredientMapping.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import ingredient_mapping_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class IngredientMappingService:
    """Camada de negócio para Mapeamento de Ingrediente."""

    def list(self, *, include_deleted: bool = False):
        query = IngredientMapping.query
        if not include_deleted:
            query = query.filter(IngredientMapping.is_deleted.is_(False))
        return query.order_by(IngredientMapping.id.asc()).all()

    def get_by_id(self, id: int) -> "IngredientMapping | None":
        return db.session.get(IngredientMapping, id)

    def create(self, data: dict) -> ServiceResult:
        obj = IngredientMapping()
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar IngredientMapping: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar IngredientMapping id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
