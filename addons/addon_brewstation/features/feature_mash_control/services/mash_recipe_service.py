"""
addons/addon_brewstation/features/feature_mash_control/services/mash_recipe_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(mash_recipe_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(MashRecipe)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in MashRecipe.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import mash_recipe_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class MashRecipeService:
    """Camada de negócio para Receita de Brassagem."""

    def list(self, *, include_deleted: bool = False):
        query = MashRecipe.query
        if not include_deleted:
            query = query.filter(MashRecipe.is_deleted.is_(False))
        return query.order_by(MashRecipe.id.asc()).all()

    def get_by_id(self, id: int) -> "MashRecipe | None":
        return db.session.get(MashRecipe, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar MashRecipe: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar MashRecipe id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
