"""
addons/addon_brewstation/features/feature_mash_control/services/dashboard_widget_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(dashboard_widget_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(DashboardWidget)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in DashboardWidget.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import dashboard_widget_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class DashboardWidgetService:
    """Camada de negócio para Widget de Dashboard."""

    def list(self, *, include_deleted: bool = False):
        query = DashboardWidget.query
        if not include_deleted:
            query = query.filter(DashboardWidget.is_deleted.is_(False))
        return query.order_by(DashboardWidget.id.asc()).all()

    def get_by_id(self, id: int) -> "DashboardWidget | None":
        return db.session.get(DashboardWidget, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar DashboardWidget: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar DashboardWidget id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
