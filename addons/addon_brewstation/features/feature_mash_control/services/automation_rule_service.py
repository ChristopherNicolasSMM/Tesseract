"""
addons/addon_brewstation/features/feature_mash_control/services/automation_rule_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(automation_rule_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(AutomationRule)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in AutomationRule.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import automation_rule_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class AutomationRuleService:
    """Camada de negócio para Regra de Automação."""

    def list(self, *, include_deleted: bool = False):
        query = AutomationRule.query
        if not include_deleted:
            query = query.filter(AutomationRule.is_deleted.is_(False))
        return query.order_by(AutomationRule.id.asc()).all()

    def get_by_id(self, id: int) -> "AutomationRule | None":
        return db.session.get(AutomationRule, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar AutomationRule: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar AutomationRule id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
