"""
addons/addon_brewstation/features/feature_mash_control/services/recipe_ingredient_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(recipe_ingredient_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(RecipeIngredient)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in RecipeIngredient.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import recipe_ingredient_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class RecipeIngredientService:
    """Camada de negócio para Ingrediente de Receita."""

    def list(self, *, include_deleted: bool = False):
        query = RecipeIngredient.query
        if not include_deleted:
            query = query.filter(RecipeIngredient.is_deleted.is_(False))
        return query.order_by(RecipeIngredient.id.asc()).all()

    def get_by_id(self, id: int) -> "RecipeIngredient | None":
        return db.session.get(RecipeIngredient, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar RecipeIngredient: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar RecipeIngredient id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
