"""
addons/addon_brewstation/features/feature_mash_control/services/brew_session_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(brew_session_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(BrewSession)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in BrewSession.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import brew_session_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class BrewSessionService:
    """Camada de negócio para Sessão de Brassagem."""

    def list(self, *, include_deleted: bool = False):
        query = BrewSession.query
        if not include_deleted:
            query = query.filter(BrewSession.is_deleted.is_(False))
        return query.order_by(BrewSession.id.asc()).all()

    def get_by_id(self, id: int) -> "BrewSession | None":
        return db.session.get(BrewSession, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar BrewSession: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar BrewSession id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
