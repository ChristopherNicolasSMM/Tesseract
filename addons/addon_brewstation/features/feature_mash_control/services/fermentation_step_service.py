"""
addons/addon_brewstation/features/feature_mash_control/services/fermentation_step_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(fermentation_step_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(FermentationStep)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in FermentationStep.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import fermentation_step_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class FermentationStepService:
    """Camada de negócio para Etapa de Fermentação."""

    def list(self, *, include_deleted: bool = False):
        query = FermentationStep.query
        if not include_deleted:
            query = query.filter(FermentationStep.is_deleted.is_(False))
        return query.order_by(FermentationStep.id.asc()).all()

    def get_by_id(self, id: int) -> "FermentationStep | None":
        return db.session.get(FermentationStep, id)

    def create(self, data: dict) -> ServiceResult:
        obj = FermentationStep()
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar FermentationStep: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar FermentationStep id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
