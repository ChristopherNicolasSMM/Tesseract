"""
addons/addon_brewstation/features/feature_mash_control/services/dashboard_layout_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(dashboard_layout_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(DashboardLayout)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in DashboardLayout.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import dashboard_layout_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class DashboardLayoutService:
    """Camada de negócio para Layout de Dashboard."""

    def list(self, *, include_deleted: bool = False):
        query = DashboardLayout.query
        if not include_deleted:
            query = query.filter(DashboardLayout.is_deleted.is_(False))
        return query.order_by(DashboardLayout.id.asc()).all()

    def get_by_id(self, id: int) -> "DashboardLayout | None":
        return db.session.get(DashboardLayout, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar DashboardLayout: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar DashboardLayout id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
