"""
addons/addon_brewstation/features/feature_mash_control/services/brew_session_step_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(brew_session_step_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(BrewSessionStep)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in BrewSessionStep.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_mash_control.services import brew_session_step_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class BrewSessionStepService:
    """Camada de negócio para Passo da Sessão."""

    def list(self, *, include_deleted: bool = False):
        query = BrewSessionStep.query
        if not include_deleted:
            query = query.filter(BrewSessionStep.is_deleted.is_(False))
        return query.order_by(BrewSessionStep.id.asc()).all()

    def get_by_id(self, id: int) -> "BrewSessionStep | None":
        return db.session.get(BrewSessionStep, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar BrewSessionStep: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar BrewSessionStep id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
