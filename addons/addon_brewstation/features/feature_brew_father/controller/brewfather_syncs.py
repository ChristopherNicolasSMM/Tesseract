"""
addons/addon_brewstation/features/feature_brew_father/controller/brewfather_syncs.py

Rotas web (HTML) — gerado pelo CrudGen. NÃO editar diretamente.
Customizações via brewfather_syncs_hooks.py (nunca sobrescrito).
"""
import csv
import importlib
import io
...
# importava/chamava brewfather_syncs_hooks.py de verdade, só tinha o
...
try:
    from addons.addon_brewstation.features.feature_brew_father.controller import brewfather_syncs_hooks as _hooks
except ImportError:
    _hooks = None
...
brewfather_syncs_bp = Blueprint(
    "brewfather_syncs", __name__, url_prefix="/brewstation/brewfather-syncs"
)
...
_READONLY_FIELDS = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(BrewFatherSync)
_EDITABLE_FIELDS = [c.name for c in BrewFatherSync.__table__.columns if c.name not in _READONLY_FIELDS]
...
_BOOLEAN_FIELDS = [
    c.name for c in BrewFatherSync.__table__.columns
...
_CHOICES_FIELDS = [f["field"] for f in get_choices_fields(BrewFatherSync) if f["field"] in _EDITABLE_FIELDS]
...
_WEAK_REFS = [wr for wr in get_weak_refs(BrewFatherSync) if wr["field"] in _EDITABLE_FIELDS]
...
_ENUM_FIELDS = [ef for ef in get_enum_fields(BrewFatherSync) if ef["field"] in _EDITABLE_FIELDS]
...
for _field, _rules in get_model_metadata(BrewFatherSync).get("validations", {}).items():
...
for _field, _html_attrs in html_types_for_model(BrewFatherSync, _EDITABLE_FIELDS).items():
...
_FIELD_LABELS: dict = get_field_labels(BrewFatherSync)

_LIST_KEY = "brewfather_syncs"
...
def _inactivate_many_delegated(ids: list[int]) -> dict:
    ...
    for id in ids:
        obj = db.session.get(BrewFatherSync, id)
...
def _apply_filters(query):
    ...
    if search:
        search_field = getattr(BrewFatherSync, _SUMMARY_FIELD, None)
    ...
    for field in _BOOLEAN_FIELDS:
        ...
        if value in ("true", "false"):
            query = query.filter(getattr(BrewFatherSync, field).is_(value == "true"))

    for field in _CHOICES_FIELDS:
        value = request.args.get(f"filter_{field}")
        if value:
            query = query.filter(getattr(BrewFatherSync, field) == value)
    ...
    for _ef in _ENUM_FIELDS:
        ...
        if value:
            query = query.filter(getattr(BrewFatherSync, _ef["field"]) == value)
...
def _choices_options() -> dict:
    ...
    options = {}
    for field in _CHOICES_FIELDS:
        column = getattr(BrewFatherSync, field)
...
def _manage_context(submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    query = _apply_filters(BrewFatherSync.query.filter(BrewFatherSync.is_deleted.is_(False)))

    total = query.count()
    items = query.order_by(BrewFatherSync.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
    pages = max(1, (total + per_page - 1) // per_page)

    return dict(
        items=items, label="Sincronização BrewFather", fields=_EDITABLE_FIELDS, summary_field=_SUMMARY_FIELD,
...
@brewfather_syncs_bp.route("/", methods=["GET"])
@login_required
@permission_required("brewfather_syncs.list")
def manage():
    return render_template("brewfather_syncs/manage.html", **_manage_context())


@brewfather_syncs_bp.route("/column-prefs", methods=["POST"])
@login_required
@permission_required("brewfather_syncs.list")
...
@brewfather_syncs_bp.route("/export.csv", methods=["GET"])
@login_required
@permission_required("brewfather_syncs.list")
def export_csv():
    query = _apply_filters(BrewFatherSync.query.filter(BrewFatherSync.is_deleted.is_(False)))
    items = query.order_by(BrewFatherSync.id.desc()).all()
...
@brewfather_syncs_bp.route("/export.xlsx", methods=["GET"])
@login_required
@permission_required("brewfather_syncs.list")
def export_xlsx():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Sincronização BrewFather"[:31]  # limite do Excel pro nome da aba
...
def _detail_context(item, submitted_data: dict | None = None, form_error: str | None = None) -> dict:
    ...
    return dict(
        item=item, label="Sincronização BrewFather", fields=_EDITABLE_FIELDS,
        field_rules=_get_field_rules(),
        weak_ref_fields=_WEAK_REF_FIELDS,
...
@brewfather_syncs_bp.route("/<int:id>", methods=["GET"])
@login_required
...
def detail(id: int):
    ...
    if not item:
        ...
        return redirect(url_for("brewfather_syncs.manage"))
    return render_template("brewfather_syncs/detail.html", **_detail_context(item))
...
@brewfather_syncs_bp.route("/", methods=["POST"])
@login_required
...
def create():
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao criar BrewFatherSync: %s", e)
...
@brewfather_syncs_bp.route("/<int:id>", methods=["POST"])
@login_required
...
def update(id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.warning("Erro inesperado ao atualizar BrewFatherSync id=%s: %s", id, e)
        success, error = False, str(e)
    if not success:
        # Mesmo raciocínio do create() acima — re-renderiza com os
        # valores submetidos em vez de redirect() + perder tudo.
        return render_template(
            "brewfather_syncs/detail.html",
            **_detail_context(item, submitted_data=submitted, form_error=error),
        )
    flash("Salvo com sucesso.", "success")
    return redirect(url_for("brewfather_syncs.detail", id=id))


@brewfather_syncs_bp.route("/<int:id>/trash", methods=["POST"])
@login_required
...
@brewfather_syncs_bp.route("/<int:id>/restore", methods=["POST"])
@login_required
...
@brewfather_syncs_bp.route("/<int:id>/delete-permanent", methods=["POST"])
@login_required
...
@brewfather_syncs_bp.route("/bulk-trash", methods=["POST"])
@login_required
@permission_required("brewfather_syncs.trash")
def bulk_trash():
...
@brewfather_syncs_bp.route("/bulk-inactivate", methods=["POST"])
@login_required
@permission_required("brewfather_syncs.update")
def bulk_inactivate():
    from flask import jsonify
