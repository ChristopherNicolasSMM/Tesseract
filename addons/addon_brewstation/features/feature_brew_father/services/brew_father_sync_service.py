"""
addons/addon_brewstation/features/feature_brew_father/services/brew_father_sync_service.py

Gerado pelo CrudGen — NÃO editar diretamente. Customizações via hooks
(brew_father_sync_service_hooks.py, nunca sobrescrito).
"""
...
_READONLY = {"id", "created_at", "updated_at", "is_deleted", "deleted_at"} | get_readonly_fields(BrewFatherSync)
...
_ATIVO_FIELD_NAME = next(
    (f for f in ("ativo", "is_active") if f in BrewFatherSync.__table__.columns.keys()),
    None,
)

try:
    from addons.addon_brewstation.features.feature_brew_father.services import brew_father_sync_service_hooks as _hooks
except ImportError:
    _hooks = None
...
class BrewFatherSyncService:
    """Camada de negócio para Sincronização BrewFather."""

    def list(self, *, include_deleted: bool = False):
        query = BrewFatherSync.query
        if not include_deleted:
            query = query.filter(BrewFatherSync.is_deleted.is_(False))
        return query.order_by(BrewFatherSync.id.asc()).all()

    def get_by_id(self, id: int) -> "BrewFatherSync | None":
        return db.session.get(BrewFatherSync, id)
    ...
    def create(self, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao criar BrewFatherSync: %s", e)
    ...
    def update(self, id: int, data: dict) -> ServiceResult:
        ...
        try:
            ...
            db.session.rollback()
            logger.warning("Erro ao atualizar BrewFatherSync id=%s: %s", id, e)
            return ServiceResult(success=False, error=_friendly_db_error(e), code=422)
